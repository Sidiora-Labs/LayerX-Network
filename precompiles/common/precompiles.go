...
func HandlePaymentUhpx(ctx sdk.Context, precompileAddr sdk.AccAddress, payer sdk.AccAddress, value *big.Int, bankKeeper putils.BankKeeper, evmKeeper putils.EVMKeeper, hooks *tracing.Hooks, depth int) (sdk.Coin, error) {
	uhpx, wei := state.SplitUhpxWeiAmount(value)
	if !wei.IsZero() {
	...
	coin := sdk.NewCoin(sdk.MustGetBaseDenom(), uhpx)
...
func HandlePaymentUhpxWei(ctx sdk.Context, precompileAddr sdk.AccAddress, payer sdk.AccAddress, value *big.Int, bankKeeper putils.BankKeeper, evmKeeper putils.EVMKeeper, hooks *tracing.Hooks, depth int) (sdk.Int, sdk.Int, error) {
	uhpx, wei := state.SplitUhpxWeiAmount(value)
	// refund payer because the following precompile logic will debit the payments from payer's account
	// this creates a new event manager to avoid surfacing these as cosmos events
	if err := bankKeeper.SendCoinsAndWei(ctx.WithEventManager(sdk.NewEventManager()), precompileAddr, payer, uhpx, wei); err != nil {
...
func GetPaxAddressByEvmAddress(ctx sdk.Context, evmAddress common.Address, evmKeeper putils.EVMKeeper) (sdk.AccAddress, error) {
	paxAddr, associated := evmKeeper.GetPaxAddress(ctx, evmAddress)
	if !associated {
...
func GetPaxAddressFromArg(ctx sdk.Context, arg interface{}, evmKeeper putils.EVMKeeper) (sdk.AccAddress, error) {
	addr := arg.(common.Address)
	if addr == (common.Address{}) {
		return nil, errors.New("invalid addr")
	}
	return GetPaxAddressByEvmAddress(ctx, addr, evmKeeper)
}
