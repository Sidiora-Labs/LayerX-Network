...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='credit-', dir=evidence) as temporary:
        work = Path(temporary)
        descriptor = os.open(work / 'genesis', os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
        with os.fdopen(descriptor, 'wb') as output:
            output.write(os.urandom(32))
        descriptor = os.open(work / 'actor', os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
        with os.fdopen(descriptor, 'wb') as output:
            output.write(hashlib.sha256(b'LayerX/light-credit-vector/actor').digest())
        ...
        if (len(profile) != 223 or profile[:5] != b'LXBC3' or profile[97:129] != asset
                ...
                or credit[327:359] != hashlib.sha256(credit[363:]).digest()):
            raise ValueError('light-client custody vector identity')
        (work / 'profile').write_bytes(profile)
        (work / 'credit').write_bytes(credit)
