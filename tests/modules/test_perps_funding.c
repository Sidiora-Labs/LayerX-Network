...
static lx_account *bob_margin_b;
...
static lxp_result tick_run(const char *did)
{
    ...
    if (lx_perps_tick_command_encode(&command, payload) != LXP_OK)
        return LXP_FATAL_INVARIANT;
    return run(LX_PERPS_FUNDING_TICK, did, payload, sizeof(payload), NULL,
               NULL);
}
...
int main(void)
{
    ...
    index.magnitude = (lxp_u128){ 0U, 10U };
    if (fixture_init() != 0 || kernel_start() != 0 ||
        account_add("agent:did:key:alice:main", LX_ACCOUNT_OPEN_CREDIT, 5000U,
                    &alice_main) != 0 ||
        ...
        account_add("agent:did:key:bob:margin:a", LX_ACCOUNT_OPEN_CREDIT, 0U,
                    &bob_margin_a) != 0 ||
        account_add("agent:did:key:bob:margin:b", LX_ACCOUNT_OPEN_CREDIT,
                    1000U, &bob_margin_b) != 0)
        return 1;
    (void)memcpy(position_id, alice_margin_a->id, 32U);
    market_defaults(1U, &market);
    if (lx_perps_pnl_compute(LX_PERPS_SIDE_BUY, (lxp_u128){ 0U, 100U },
                             (lxp_u128){ 0U, 99U }, (lxp_u128){ 0U, 1U },
                             (lxp_u128){ 0U, 3U }, &pnl) != LXP_OK ||
        !pnl.negative || pnl.magnitude.lo != 1U ||
        lx_perps_pnl_compute(LX_PERPS_SIDE_SELL, (lxp_u128){ 0U, 100U },
                             (lxp_u128){ 0U, 99U }, (lxp_u128){ 0U, 3U },
                             (lxp_u128){ 0U, 1U }, &pnl) != LXP_OK ||
        pnl.negative || pnl.magnitude.lo != 3U ||
        lx_perps_funding_rate(&market, (lxp_u128){ 0U, 105U },
                              (lxp_u128){ 0U, 100U }, 1000U,
                              &rate) != LXP_OK ||
        rate.negative || rate.magnitude.lo != 500U ||
        lx_perps_funding_rate(&market, (lxp_u128){ 0U, 95U },
                              (lxp_u128){ 0U, 100U }, 1000U,
                              &rate) != LXP_OK ||
        !rate.negative || rate.magnitude.lo != 500U ||
        lx_perps_funding_index_update(index, rate, 2U, &updated) != LXP_OK ||
        !updated.negative || updated.magnitude.lo != 990U)
        return 1;
    if (market_create(&market, admin_did) != LXP_OK ||
        oracle_push(oracle_seed, 1U, 1U, 100U, 1000U) != LXP_OK ||
        position_open(10U, bob_margin_a, LX_PERPS_SIDE_SELL, 10U, 100U,
                      "did:key:bob") != LXP_OK ||
        position_open(11U, alice_margin_a, LX_PERPS_SIDE_BUY, 10U, 100U,
                      "did:key:alice") != LXP_OK ||
        order_place(1U, alice_margin_b, LX_PERPS_SIDE_BUY, 100U, 1U,
                    "did:key:alice") != LXP_OK ||
        order_place(2U, bob_margin_b, LX_PERPS_SIDE_SELL, 120U, 1U,
                    "did:key:bob") != LXP_OK)
        return 1;
    if (ctx_open() != 0 ||
        ...
        !lxp_u128_is_zero(funding.funding_index.magnitude) ||
        tick_run(admin_did) != LXP_ERR_NOT_YET_VALID)
        return 1;
    batch_timestamp = 2000U;
    if (tick_run(admin_did) != LXP_OK)
        return 1;
    if (ctx_open() != 0 ||
        lx_perps_funding_state_lookup(&ctx, market_id, &funding) != LXP_OK ||
        ...
        tick_run(admin_did) != LXP_ERR_NOT_YET_VALID)
        return 1;
    if (position_close(alice_margin_a, "did:key:alice") != LXP_OK ||
        position_close(bob_margin_a, "did:key:bob") != LXP_OK)
        return 1;
