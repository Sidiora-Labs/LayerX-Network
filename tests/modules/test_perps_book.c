...
typedef struct order_visit_state {
    size_t count;
    uint64_t remaining_total;
} order_visit_state;

static lx_account *alice_margin_a;
static lx_account *alice_margin_b;
static lx_account *bob_margin_a;

static lxp_result order_visit(const lx_perps_order *order, void *user)
{
    order_visit_state *state = (order_visit_state *)user;
    ++state->count;
    state->remaining_total += order->remaining.lo;
    return LXP_OK;
}
...
static lxp_result order_cancel(uint8_t order_id, const char *did)
{
    lx_perps_cancel_command command;
    uint8_t payload[LX_PERPS_CANCEL_PAYLOAD_BYTES];
    (void)memset(&command, 0, sizeof(command));
    command.market_id[0] = 1U;
    command.order_id[0] = order_id;
    if (lx_perps_cancel_command_encode(&command, payload) != LXP_OK)
        return LXP_FATAL_INVARIANT;
    return run(LX_PERPS_ORDER_CANCEL, did, payload, sizeof(payload), NULL,
               NULL);
}
...
int main(void)
{
    ...
    if (market_create(&market, admin_did) != LXP_OK ||
        ...
        return 1;
    if (order_place(1U, alice_margin_b, LX_PERPS_SIDE_SELL, 100U, 3U,
                    "did:key:alice") != LXP_OK ||
        order_place(2U, alice_margin_a, LX_PERPS_SIDE_SELL, 101U, 4U,
                    "did:key:alice") != LXP_OK ||
        order_place(2U, alice_margin_a, LX_PERPS_SIDE_SELL, 101U, 4U,
                    "did:key:alice") != LXP_ERR_SEQUENCE_REUSED ||
        order_place(4U, alice_margin_a, LX_PERPS_SIDE_SELL, 101U, 4U,
                    "did:key:bob") != LXP_ERR_UNAUTHORIZED_DEBIT ||
        order_place(5U, bob_margin_a, LX_PERPS_SIDE_BUY, 100U, 1000U,
                    "did:key:bob") != LXP_ERR_MARGIN_INSUFFICIENT ||
        order_place(3U, bob_margin_a, LX_PERPS_SIDE_BUY, 102U, 6U,
                    "did:key:bob") != LXP_OK)
        return 1;
    if (ctx_open() != 0 ||
        order_state(1U, &order) != LXP_ERR_UNKNOWN_FIELD ||
        order_state(3U, &order) != LXP_ERR_UNKNOWN_FIELD ||
        order_state(4U, &order) != LXP_ERR_UNKNOWN_FIELD ||
        order_state(2U, &order) != LXP_OK || !order.active ||
        order.remaining.hi != 0U || order.remaining.lo != 1U ||
        order.quantity.lo != 4U || order.price.lo != 101U ||
    ...
    if (lx_perps_book_init(&book) != LXP_OK || book.count != 0U ||
        ...
        book.count != 1U ||
        lx_perps_book_iter(&book, order_visit, &visits) != LXP_OK ||
        visits.count != 1U || visits.remaining_total != 1U)
        return 1;
    if (order_cancel(2U, "did:key:bob") != LXP_ERR_UNAUTHORIZED_DEBIT ||
        order_cancel(9U, "did:key:alice") != LXP_ERR_UNKNOWN_FIELD ||
        order_cancel(2U, "did:key:alice") != LXP_OK ||
        order_cancel(2U, "did:key:alice") != LXP_ERR_UNKNOWN_FIELD)
        return 1;
    if (ctx_open() != 0 ||
