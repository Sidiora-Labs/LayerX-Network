                             ...
                             const char *did)
{
    ...
    uint8_t payload[LX_PERPS_ADL_PAYLOAD_MAX_BYTES];
    size_t length = 0U;
    size_t i;
    (void)memset(&command, 0, sizeof(command));
    command.market_id[0] = 1U;
...
int main(void)
{
    ...
    if (fixture_init() != 0 || kernel_start() != 0 ||
        lxp_ledger_bootstrap_balance(insurance_account, quote_asset_id,
                                     (lxp_u128){ 0U, 850U }, 0U) != LXP_OK ||
    ...
    if (ctx_open() != 0 || !balance_is(bob_margin, 0U) ||
        ...
        return 1;
    if (lx_perps_deficit_record(&deficits, market_id, insurance_account->id,
                                (lxp_u128){ 0U, 71U }, 10U) != LXP_OK ||
        lx_perps_deficit_record(&deficits, market_id, insurance_account->id,
                                (lxp_u128){ 0U, 29U }, 11U) != LXP_OK ||
        deficits.count != 1U || deficits.deficits[0].amount.lo != 100U ||
        deficits.deficits[0].recorded_at_sequence != 11U ||
        lx_perps_deficit_record(&deficits, market_id, insurance_account->id,
                                (lxp_u128){ 0U, 0U }, 12U) !=
            LXP_ERR_NON_CANONICAL)
        return 1;
    if (lxp_ledger_bootstrap_balance(insurance_account, quote_asset_id,
                                     (lxp_u128){ 0U, 300U }, 0U) != LXP_OK ||
        ctx_open() != 0 ||
        lx_perps_insurance_cover(&ctx, insurance_account, liquidity_account,
                                 &asset_state, (lxp_u128){ 0U, 1000U },
                                 context, &receipt) !=
            LXP_ERR_INSUFFICIENT_BALANCE ||
        !balance_is(insurance_account, 300U) ||
        lx_perps_insurance_cover(&ctx, insurance_account, liquidity_account,
                                 &asset_state, (lxp_u128){ 0U, 300U },
                                 context, &receipt) != LXP_OK ||
        !balance_is(insurance_account, 0U) ||
        !balance_is(liquidity_account, 3300U) ||
