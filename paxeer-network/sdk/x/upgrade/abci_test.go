...
func setupTest(t *testing.T, height int64) TestSuite {
	db := dbm.NewMemDB()
	app := paxapp.SetupWithDB(t, db, false, false, false)
	genesisState := paxapp.NewDefaultGenesisState(app.AppCodec())
	stateBytes, err := json.MarshalIndent(genesisState, "", "  ")
	if err != nil {
		panic(err)
	}
	blockTime := time.Now().UTC()
	app.InitChain(t.Context(), &abci.RequestInitChain{Time: blockTime, AppStateBytes: stateBytes})

	s.keeper = app.UpgradeKeeper
	s.ctx = app.BaseApp.NewContext(false, tmproto.Header{Height: height, Time: blockTime})
