...
func run() error {
	...
	if err := flags.Parse(os.Args[1:]); err != nil { return err }
	if flags.NArg() != 0 || (*state == "") != (*key == "") { return fmt.Errorf("history state and authority must be supplied together") }
	...
	var result any
	if *state == "" {
		if request.Operation != "" && request.Operation != "verify" { return fmt.Errorf("history operation requires durable state") }
		...
		case "status": result, err = history.Status(now)
		case "advance":
			err = history.Advance(request.Bundle.History, now)
			if err == nil { result, err = history.Status(now) }
		case "verify": result, err = history.Verify(request, now)
		case "export": result, err = history.Export()
		default: return fmt.Errorf("unsupported history operation")
		}
	}
	if err != nil { return err }
