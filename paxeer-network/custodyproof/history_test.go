...
func realHistory(t *testing.T) (*Request, *History, string, time.Time) {
	...
	if err := os.WriteFile(key, bytes.Repeat([]byte{0x55}, 32), 0600); err != nil {
		t.Fatal(err)
	}
...
func advanceReal(t *testing.T, history *History, entries []LightBlock, now time.Time) {
	...
	for start := 0; start < len(entries); start += 7 {
		if err := history.Advance(entries[start:min(start+7, len(entries))], now); err != nil {
			t.Fatal(err)
		}
	}
}
...
func TestRealHistoryRestartAndExpiredCatchup(t *testing.T) {
	...
	if historyStatus(t, history, now).Height != 0 || !historyStatus(t, history, now).HistoricalCatchup {
		t.Fatal("genesis status")
	}
	advanceReal(t, history, request.Bundle.History, now.Add(2*TrustingPeriod))
	if !historyStatus(t, history, now.Add(2*TrustingPeriod)).HistoricalCatchup {
		t.Fatal("expired cache concealed")
	}
	if _, err := history.Verify(cachedRequest(request), now.Add(2*TrustingPeriod)); err == nil {
		t.Fatal("expired live proof accepted")
	}
	...
	if result.DepositID != request.Expected.DepositID {
		t.Fatal("recovered deposit binding")
	}
	exported, err := reopened.Export()
	if err != nil || len(exported) != len(request.Bundle.History) {
		t.Fatal("public history export", err)
	}
}
...
func TestRealHistoryRecoversCommittedChunkBeforeAnchor(t *testing.T) {
	...
	if count < 3 {
		t.Fatal("real fixture requires adjacent history")
	}
	...
	if err := history.Advance(request.Bundle.History[count-1:], now); err != nil {
		t.Fatal(err)
	}
	if err := history.Close(); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(anchor, prior, 0600); err != nil {
		t.Fatal(err)
	}
	...
	if historyStatus(t, recovered, now).Height != int64(count) {
		t.Fatal("committed history regressed")
	}
	...
	for _, fault := range []string{"rollback", "corruption", "anchor-corruption", "missing-anchor", "deleted-database", "missing-retained-record"} {
		t.Run(fault, func(t *testing.T) {
			...
			case "rollback":
				...
				if err := history.database.Put([]byte("head"), older, &opt.WriteOptions{Sync: true}); err != nil {
					t.Fatal(err)
				}
			...
			case "corruption":
				if err := history.database.Put([]byte("head"), []byte("corrupt"), &opt.WriteOptions{Sync: true}); err != nil {
					t.Fatal(err)
				}
			}
			if err := history.Close(); err != nil {
			...
			case "anchor-corruption":
				if err := os.WriteFile(anchor, []byte("corrupt"), 0600); err != nil {
					t.Fatal(err)
				}
			case "missing-anchor":
			...
			case "deleted-database":
				if err := os.Rename(filepath.Join(history.directory, "history.db"), filepath.Join(history.directory, "retained.db")); err != nil {
					t.Fatal(err)
				}
			}
			...
			if err == nil {
				accepted.Close()
				t.Fatal("damaged durable history accepted")
			}
		})
	}
}
...
func TestRealHistoryRefusesGapsEquivocationAndInvalidQuorum(t *testing.T) {
	...
	if err := history.Advance(request.Bundle.History[1:2], now); err == nil {
		t.Fatal("caller checkpoint accepted")
	}
	if err := history.Advance(nil, now); err == nil {
		t.Fatal("empty work chunk accepted")
	}
	oversized := make([]LightBlock, MaxHistoryChunk+1)
	if err := history.Advance(oversized, now); err == nil {
		t.Fatal("unbounded work chunk accepted")
	}
	advanceReal(t, history, request.Bundle.History, now)
	changed := realRequest(t)
	changed.Bundle.History[0].Commit.Header.AppHash[0] ^= 1
	if err := history.Advance(changed.Bundle.History[:1], now); err == nil {
		t.Fatal("equivocating cached header accepted")
	}
	changed = realRequest(t)
	...
	if !damaged {
		t.Fatal("real fixture has no signatures")
	}
	if err := history.Advance(changed.Bundle.History[:1], now); err == nil {
		t.Fatal("invalid cached quorum accepted")
	}
	changed = realRequest(t)
	changed.Bundle.Genesis[0] ^= 1
	if _, err := history.Verify(cachedRequest(changed), now); err == nil {
		t.Fatal("changed raw genesis accepted through cache")
	}
	changed = realRequest(t)
	changed.Bundle.History = changed.Bundle.History[:len(changed.Bundle.History)-1]
	if _, err := history.Verify(changed, now); err == nil {
		t.Fatal("history ending before final head accepted")
	}
	if _, err := history.Verify(cachedRequest(request), now); err != nil {
		t.Fatal("refusal corrupted committed state", err)
	}
}
...
func TestRealHistoryRefusesChangedAuthorityAndUnsafePaths(t *testing.T) {
	...
	if accepted, err := OpenHistory(history.directory, key, request.Expected, request.Bundle.Genesis, now); err == nil {
		accepted.Close()
		t.Fatal("public authority seed accepted")
	}
}
...
func TestRealHistoryRefusesFIFOAuthorityAndHighWater(t *testing.T) {
	for _, target := range []string{"authority", "high-water"} {
		t.Run(target, func(t *testing.T) {
			...
			path := key
			if target == "high-water" {
				path = filepath.Join(history.directory, "high-water.json")
			}
			...
			if accepted, err := OpenHistory(history.directory, key, request.Expected, request.Bundle.Genesis, now); err == nil {
				accepted.Close()
				t.Fatal("FIFO trust input accepted")
			}
		})
	}
}
...
func TestRealHistoryPoisonsFailedAnchorWriteUntilReopen(t *testing.T) {
	...
	if err := os.Rename(anchor, retained); err != nil {
		t.Fatal(err)
	}
	...
	if err := history.Advance(request.Bundle.History[count-1:], now); err == nil {
		t.Fatal("failed anchor write accepted")
	}
	if err := history.Advance(request.Bundle.History[count-1:], now); err == nil {
		t.Fatal("cached retry bypassed anchor failure")
	}
	if _, err := history.Verify(cachedRequest(request), now); err == nil {
		t.Fatal("proof issued without durable anchor")
	}
	if _, err := history.Export(); err == nil {
		t.Fatal("history exported after durability failure")
	}
	if _, err := history.Status(now); err == nil {
		t.Fatal("successful status after durability failure")
	}
	if err := history.Close(); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.Rename(retained, anchor); err != nil {
		t.Fatal(err)
	}
	recovered, err := OpenHistory(history.directory, key, request.Expected, request.Bundle.Genesis, now)
	if err != nil {
	...
	defer recovered.Close()
	if historyStatus(t, recovered, now).Height != int64(count) {
		t.Fatal("durable committed header lost")
	}
	if _, err := recovered.Verify(cachedRequest(request), now); err != nil {
		t.Fatal(err)
	}
