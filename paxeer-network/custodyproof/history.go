...
func privateDirectory(path string) error {
	...
	for current := absolute; current != filepath.Dir(current); current = filepath.Dir(current) {
		...
		if err != nil || !info.IsDir() || info.Mode()&os.ModeSymlink != 0 {
			return errors.New("history directory identity")
		}
	}
...
func (history *History) seal(record *historyRecord) ([]byte, error) {
	...
	if err != nil || len(encoded) > MaxHistoryRecord {
		return nil, errors.New("history record bound")
	}
	return encoded, nil
}

func (history *History) decode(encoded []byte) (*historyRecord, error) {
	if len(encoded) == 0 || len(encoded) > MaxHistoryRecord {
		return nil, errors.New("history record bound")
	}
	...
	if err != nil || !bytes.Equal(encoded, canonical) {
		return nil, errors.New("noncanonical history record")
	}
	...
	if record.Height == 0 {
		if record.Block != nil {
			return nil, errors.New("history genesis record")
		}
...
func (history *History) get(height int64) (*historyRecord, error) {
	...
	if err != nil || record.Height != height {
		return nil, errors.New("authenticated history height")
	}
	return record, nil
}
...
func (history *History) writeHighWater(encoded []byte) error {
	...
	if err := os.Rename(name, filepath.Join(history.directory, "high-water.json")); err != nil {
		return err
	}
...
func OpenHistory(directory, keyPath string, expected Expected, genesisBytes []byte, now time.Time) (*History, error) {
	...
	if err != nil || len(seed) != ed25519.SeedSize {
		return nil, errors.New("history attestor key")
	}
	...
	if statErr != nil && anchorErr == nil {
		return nil, errors.New("history database rollback or deletion")
	}
...
func (history *History) checkIndex() error {
	...
	for iterator.Next() {
		count++
		if count > MaxHistory+3 {
			return errors.New("history retained record bound")
		}
		key := iterator.Key()
		if bytes.Equal(key, []byte("head")) {
			continue
		}
...
func (history *History) openHead(anchor []byte, anchored bool) error {
	...
	if !anchored {
		if head.Height != 0 {
			return errors.New("missing authenticated history high-water")
		}
	} else {
		...
		for height := highWater.Height; height <= head.Height; height++ {
			...
			if height == highWater.Height {
				if !bytes.Equal(record.Signature, highWater.Signature) {
					return errors.New("history high-water equivocation")
				}
		...
		if !bytes.Equal(previous.Signature, head.Signature) {
			return errors.New("history head disagreement")
		}
	}
	history.head = head
...
func (history *History) Status(now time.Time) (HistoryStatus, error) {
	if history.failure != nil {
		return HistoryStatus{}, history.failure
	}
...
func (history *History) Advance(entries []LightBlock, now time.Time) error {
	...
	if len(entries) == 0 || len(entries) > MaxHistoryChunk {
		return errors.New("history work chunk bound")
	}
	batch := new(leveldb.Batch)
	head := history.head
	...
	for index := range entries {
		entry := &entries[index]
		if entry.Commit.Header == nil {
			return errors.New("missing history header")
		}
		...
		if height <= head.Height {
			...
			if !bytes.Equal(record.Block.Commit.Hash(), entry.Commit.Hash()) {
				return errors.New("history equivocation")
			}
			if height > 1 {
			...
			if err := verifyEntry(previous, entry, history.genesis, history.validators, now); err != nil {
				return err
			}
			continue
		}
		...
		if head.Height > 0 {
			previous = &head.Block.Commit.SignedHeader
		}
		if err := verifyEntry(previous, entry, history.genesis, history.validators, now); err != nil {
			return err
		}
	...
	if head.Height == history.head.Height {
		return nil
	}
...
func (history *History) Verify(request *Request, now time.Time) (*Result, error) {
	...
	if err := history.Advance(request.Bundle.History, now); err != nil {
		return nil, err
	}
...
func (history *History) Export() ([]LightBlock, error) {
	if history.failure != nil {
		return nil, history.failure
	}
	if history.head.Height <= 0 || history.head.Height > MaxHistory {
		return nil, errors.New("individual history export bound")
	}
