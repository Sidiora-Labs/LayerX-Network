...
func TestRealStateCreditAndClockBounds(t *testing.T) {
	...
	if _, err := Verify(request, last.Add(TrustingPeriod)); err == nil { t.Fatal("expired live head accepted") }
	if _, err := Verify(request, last.Add(-MaxClockDrift)); err == nil { t.Fatal("future header accepted") }
}
...
func TestRealStateCreditRefusesMissingAuthority(t *testing.T) {
	...
	if _, err := Verify(request, now); err == nil { t.Fatal("missing validator key accepted") }
	request = realRequest(t)
	request.Bundle.History[0].Commit.Commit = nil
	if _, err := Verify(request, now); err == nil { t.Fatal("missing commit accepted") }
}
...
func TestJSONRequestRefusesAmbiguousFields(t *testing.T) {
	for _, encoded := range []string{`{"expected":{},"expected":{},"bundle":{}}`,
		...
		if _, err := Decode([]byte(encoded)); err == nil { t.Fatal("ambiguous request accepted") }
	}
}
