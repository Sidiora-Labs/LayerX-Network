...
Image-only commands are invoked on the script, not through Make: `bash platform/hosted/tests/beta-cluster.sh images` builds or pulls the shared table (`platform/hosted/tests/beta-cluster.sh:1917-1921`); `bash platform/hosted/tests/beta-cluster.sh publish-images` tags and pushes SHA and `beta` tags to GHCR (`platform/hosted/tests/beta-cluster.sh:1923-1924`; `platform/hosted/tests/publish-images.sh`). `.github/workflows/publish-images.yml` runs those two commands on `main`.

When `platform-beta-cluster-up` or `platform-beta-cluster-down` is among the make goals, make sets `.NOTPARALLEL` (`platform/Makefile.inc:180-182`). Smoke consumes the env file that `up` writes (`platform/Makefile.inc:3`, `platform/hosted/tests/beta-cluster.sh:63`, `platform/hosted/tests/beta-cluster.sh:1640-1672`).

## Bring-up order

`beta-cluster.sh up` runs `beta_cluster_up` (`platform/hosted/tests/beta-cluster.sh:5`, `platform/hosted/tests/beta-cluster.sh:1769`). After host-tool, Foundry, custody-profile, disk, `tools_install`, image, cluster, `node_boundary_install`, CA, and secret steps, it renders manifests, publishes the builder release, and applies trusted-boundary manifests before the testnet, gateway, and registry manifests (`platform/hosted/tests/beta-cluster.sh:42-48`, `platform/hosted/tests/beta-cluster.sh:1078-1101`). The gateway is the only hosted surface serving public `/v1` routes to humans and SDKs; the Human HTTPS API is a separate in-cluster Service. See [Hosted gateway](HostedGateway.md) and [Hosted Human](HostedHuman.md). The developer manifest is `platform/hosted/webhooks/deployment.yaml`; durable outbound delivery of protocol events is documented on [Hosted webhooks](HostedWebhooks.md). The registry StatefulSet those manifests apply is [Hosted registry](HostedRegistry.md) (`platform/hosted/registry/deployment.yaml:2-3`, `platform/hosted/tests/beta-cluster.sh:1101`). The signing KMS and event sources in namespace `layerx-internal` are on [Hosted internal](HostedInternal.md) (`platform/hosted/tests/beta-cluster.sh:93`, `platform/hosted/internal/deployment.yaml:3`).

Inside that binding: the Paxeer chain starts with the generated deployer address, the node bootstraps its genesis, `deploy-contracts.sh` deploys settlement contracts from the node's genesis artifacts through the Paxeer boundary, and the resulting GuarantorBond and CheckpointRegistry addresses are published as ConfigMap `layerx-node-settlement` (`platform/hosted/tests/beta-cluster.sh:41-47`, `platform/hosted/tests/beta-cluster.sh:1247-1253`, `platform/hosted/tests/beta-cluster.sh:928-938`). `trusted_boundary_apply` applies `registry-journal.yaml`, then `paxeer.yaml`, then `identity.yaml`, then deletes any leftover standalone `layerx-human` Deployment and applies `human.yaml`, then the node bootstrap or retained node manifest (`platform/hosted/tests/beta-cluster.sh:1078-1094`).
...
`publish-images.sh` reads `build/beta-cluster/images`, requires every name in the shared table, tags `ghcr.io/sidiora-labs/<name>:<revision>` and `:beta`, pushes both, and requires the registry manifest digest to match the local image (`platform/hosted/tests/publish-images.sh:10-63`). `--check` verifies the inventory without tagging or pushing (`platform/hosted/tests/publish-images.sh:46-48`). There is no Make target for `images` or `publish-images`.
