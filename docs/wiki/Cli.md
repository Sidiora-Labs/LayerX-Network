...
| `layerx environment list` | List configured endpoint profiles (`platform/cli/src/main.rs:91-93, 684-702`) | none |
...
| `layerx key delete <name>` | Permanently delete a key from credential storage (`platform/cli/src/main.rs:130-131, 844-850`) | `name` |
| `layerx auth set` | Read an API token from stdin and save it (`platform/cli/src/main.rs:135-140, 858-865`) | token on stdin. `--environment` optional (else current; `platform/cli/src/main.rs:1350-1356`) |
| `layerx auth status` | Report whether a token exists without printing it (`platform/cli/src/main.rs:141-145, 867-878`) | `--environment` optional |
| `layerx auth delete` | Permanently delete a stored API token (`platform/cli/src/main.rs:146-150, 880-887`) | `--environment` optional |
| `layerx account create` | Register an account on the active endpoint (`platform/cli/src/main.rs:154-167, 896-926`; `platform/cli/src/account.rs:6-64`) | emulator: a local key (`--key` or default) (`platform/cli/src/account.rs:18-20`). Hosted: `--email`, `--display-name`, `--idempotency-key`; `--initial-amount` must be `0` (`platform/cli/src/account.rs:42-54`). `--initial-amount` default `0` |
| `layerx account get` | Read the active hosted profile or one emulator DID account (`platform/cli/src/main.rs:168-172, 928-932`; `platform/cli/src/account.rs:67-84`) | emulator requires `--did` (`platform/cli/src/account.rs:68-69`) |
| `layerx payment test` | Quote and commit a test payment through the active endpoint (`platform/cli/src/main.rs:176-189, 936-950`; `platform/cli/src/payment.rs:5-37`) | `--from`, `--to`, `--currency` (alias `--asset`), `--amount` (>0), `--idempotency-key` (16–128 alnum/`-`/`_`; `platform/cli/src/http.rs:366-377`) |
| `layerx receipt get <id>` | Fetch exact receipt material from the active endpoint (`platform/cli/src/main.rs:194-195, 956-964`) | `id` (path-safe; `platform/cli/src/http.rs:352-363`) |
| `layerx receipt verify` | Verify a canonical receipt against independently supplied batch facts, locally (`platform/cli/src/main.rs:196-214, 966-978`; `platform/cli/src/receipt.rs:18-52`) | `--receipt`, `--batch-id`, `--asset`, `--previous-state-root`, `--resulting-state-root`, `--sequencer-public-key` |
| `layerx program discover <program_id>` | Discover one active program from receipt-backed registry state (`platform/cli/src/main.rs:217-219, 1026-1033`; `platform/cli/src/programs.rs:628-644`) | `program_id` |
| `layerx program interface get <program_id>` | Read a canonical code-bound program interface (`platform/cli/src/main.rs:334-337, 1128-1132`; `platform/cli/src/programs.rs:647-673`) | `program_id` |
| `layerx program interface publish <program_id>` | Publish a canonical interface (`platform/cli/src/main.rs:338-344, 1133-1141`; `platform/cli/src/programs.rs:676-697`) | `program_id`, `--interface`, `--idempotency-key` |
| `layerx program build` | Compile to WASM and enforce the deterministic runtime policy locally (`platform/cli/src/main.rs:223-229, 1036-1043`; `platform/cli/src/programs.rs:122-138`) | `--manifest-path` default `Cargo.toml`; `--artifact` optional |
| `layerx program bindings` | Generate digest-bound Rust, TypeScript, and guest bindings (`platform/cli/src/main.rs:230-244, 1044-1053`; `platform/cli/src/programs.rs:40-96`) | `--interface`, `--digest`, `--code-hash`; `--output` default `bindings` |
| `layerx program deploy <artifact>` | Validate and submit a WASM artifact for receipt-backed deployment (`platform/cli/src/main.rs:245-254, 1054-1070`; `platform/cli/src/programs.rs:245-279`) | `artifact` plus lifecycle flags below. `--upgrade-authority` and `--interface` optional |
| `layerx program upgrade <artifact>` | Submit a WASM upgrade (`platform/cli/src/main.rs:255-267, 1071-1091`; `platform/cli/src/programs.rs:289-328`) | `artifact`, `--old-hash`, lifecycle flags. `--migration-hook` optional. `--interface` conflicts with `--clear-interface` (`platform/cli/src/main.rs:260-264, 1479-1493`) |
| `layerx program wind-down route` | Route a program-owned account (`platform/cli/src/main.rs:300-312, 1148-1168`) | `--account`, `--asset`, `--destination`; `--seed` default empty; lifecycle flags |
| `layerx program wind-down deprecate` | Deprecate toward an exit program (`platform/cli/src/main.rs:313-320, 1170-1181`) | `--exit-program`, `--deadline-batch`; lifecycle flags |
| `layerx program wind-down tombstone` | Tombstone the program (`platform/cli/src/main.rs:321-324, 1183-1191`) | lifecycle flags |
| `layerx program wind-down exit` | Exit a program-owned account (`platform/cli/src/main.rs:325-330, 1193-1199`) | `--account`; lifecycle flags |
| `layerx program call` | Submit calldata and render the receipt-verified result (`platform/cli/src/main.rs:270-271, 1099-1123, 1232-1285`; `platform/cli/src/programs.rs:810-838`) | `program_id`, `--fuel`, `--idempotency-key`, `--account-sequence`, `--not-before-ms`, `--expires-at-ms`. `--calldata` optional. `--fee-limit` default `0`. `--capability` repeatable. `--key` optional. Native ABI flags default ABI 2 / `layerx_call` / 16_777_216 memory (`platform/cli/src/programs.rs:724-756`; `platform/cli/src/main.rs:1496-1526`) |
| `layerx program simulate` | Execute a program call against current state without committing it (`platform/cli/src/main.rs:272-273, 1288-1341`; `platform/cli/src/programs.rs:902-919`) | same as `program call` |
| `layerx program registry get <program_id>` | Read one program's receipt-backed registry record (`platform/cli/src/main.rs:349-350, 1208-1212`; `platform/cli/src/programs.rs:554-625`) | `program_id` |
| `layerx program registry verify-source <program_id>` | Submit a source digest and location (`platform/cli/src/main.rs:351-360, 1213-1227`; `platform/cli/src/programs.rs:700-717`) | `program_id`, `--source-uri`, `--source-digest`, `--idempotency-key` |
| `layerx emulator provision` | Generate the sequencer seed and publish its trust anchor under the profile directory (`platform/cli/src/main.rs:367-372, 508-512`; `platform/cli/src/emulator.rs:223-305`) | `--force` optional; without it an existing seed or anchor is refused |
| `layerx emulator up` | Start the local real-transition gateway (`platform/cli/src/main.rs:365-366, 375-387, 513-525`) | `--sequencer-seed-file`. `--listen`, `--network-id`, `--time-ms`, `--prefund` optional |
| `layerx install mcp` | Install a payment-capable MCP server (`platform/cli/src/main.rs:390-392, 397-415, 596-612`; `platform/cli/src/install/mcp.rs:24-98`) | hosted environment only (`platform/cli/src/install/mod.rs:573-577`). `--host` values: `layerx`, `claude-code`, `claude-desktop`, `cursor`, `vscode` (`platform/cli/src/install/mod.rs:53-64`). Payment mode requires `--source-account` and `--asset` (`platform/cli/src/install/mcp.rs:142-156`) |
| `layerx install a2a` | Install a payment-capable A2A server (`platform/cli/src/main.rs:393-394, 417-437, 614-631`; `platform/cli/src/install/a2a.rs:40-119`) | hosted environment only. `--listen` default `127.0.0.1:9433` |
| `layerx mcp serve` | Serve MCP on stdin/stdout (`platform/cli/src/main.rs:439-455, 527-545`; `platform/cli/src/mcp.rs:13-56`) | `--gateway-credential`. `--environment`, `--key`, `--source-account`, `--asset`, `--read-only` optional |
| `layerx a2a serve` | Serve the agent card and task interface on loopback (`platform/cli/src/main.rs:458-478, 547-572`; `platform/cli/src/a2a.rs:89-176`) | `--gateway-credential`, `--authorization-file`. `--listen` default `127.0.0.1:9433` |
...
(`platform/cli/src/main.rs:1530-1532`).
...
| Gateway key | `install mcp` / `install a2a` provision `/v1/keys` or `/v1/keys/{id}/rotate` and `set_gateway` (`platform/cli/src/install/mod.rs:594-640, 673-690`; `platform/cli/src/credential.rs:228-236`) | `gateway` `get_password`; MCP/A2A serve load by alias (`platform/cli/src/toolset.rs:58-62`) |
...
32-byte hex seed from stdin (`platform/cli/src/main.rs:118-123`;
...
| --- | --- |
| Emulator | `emulator up` runs `layerx_platform_emulator::run` in-process (`platform/cli/src/main.rs:69-71, 513-524`). Other commands HTTP to the configured emulator endpoint. `environment use emulator` with bound inputs waits for a listener and `GET /v1/sequencer`, then refuses `network_id_mismatch` or `sequencer_trust_anchor_mismatch` (`platform/cli/src/emulator.rs:860-891`; `platform/cli/src/main.rs:727-757`) |
| Hosted gateway | `environment use testnet` or `production` stores `https://…` (non-loopback `http://` is refused; `platform/cli/src/http.rs:27-37`). Account, payment, receipt, and program commands send `Authorization: Bearer` when a token exists (`platform/cli/src/http.rs:222-227`; `platform/cli/src/main.rs:1344-1347`). MCP/A2A send `Authorization: LayerX-Key` (`platform/cli/src/http.rs:50-64, 229-231`; `platform/cli/src/toolset.rs:94-95`). Install MCP/A2A refuse the emulator (`platform/cli/src/install/mod.rs:573-577`) |
...
Receipts get `/v1/receipts/{id}` (`platform/cli/src/main.rs:963`). Program
...
(`platform/cli/src/toolset.rs:288-291`).
...
| `environment must be emulator, testnet, or production` (`command_failed`) | any other profile name (`platform/cli/src/config.rs:121-126`; `platform/cli/src/main.rs:719`) |
...
it does not contact the endpoint (`platform/cli/src/main.rs:966-978`).
