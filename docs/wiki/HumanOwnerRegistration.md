...
The producer output is `$WORK_DIR/human-evidence-input/owner-registration.json`, with native H32 `owner_account`, textual `authority` and `identity`. `platform/hosted/human/provision.py` provides producer and validator modes. The complete flow has not been qualified.

Hosted principal policy `identities[]` requires evidence `{activity_id, receipt_digest}` linked to the same principal's `activities[]`. The digest is the canonical unsigned protocol receipt digest. Retained evidence includes `receipt_hex` and `replica_document`; verification authenticates the sequencer, batch and receipt inclusion. Identity, capability, rotation and recovery assertions additionally require committed state evidence. The hosted authority can verify the native identity snapshot at batch inclusion level; capability and checkpoint policy assertions remain refused.

The daemon registers Governance for protocol 3. The genesis builder still needs the same module registration in `cmd/layerx-genesis/lxp_genesis_builder.c:materialize_snapshot`; fresh daemon startup currently refuses its mismatched snapshot with result -902. The snapshot check remains intact.

## Native Governance registration encoding

Governance ABI 1 registers module 7 ordinals 1, 2, 3, 5 and 6. The actor must already have its custody key admitted through the native bootstrap identity file. Registration certifies that existing actor/key into committed Governance state; it does not create an arbitrary new bootstrap identity.
...
Session registration is `7105 0001 || u32be(length) || canonical_2001_grant`. Only canonical session-key grants are accepted, bound to the actor's current revocation generation and signed by the owner envelope. Revoke is `7106 0003 || grant_id32 || reason:u8 || effective_sequence:u64`; its effective sequence must equal execution's sequence. No grant or policy mutation is accepted from an unrelated owner.

Governance stages its state through module KV writes. The identity key is did_id32. Session and revoke keys are respectively `05 || grant_id32` and `06 || grant_id32`. Event `7110` contains the exact 223-byte committed identity value: `LXGI1`, DID32, primary32, revocation:u64, recovery_root32, threshold:u16, pending32, rotation_begin:u64, rotation_end:u64, rotation_effective_sequence:u64, rotation_revision:u64, recovery_revision:u64, recovery_delay_seconds:u64, recovery_maximum_seconds:u64, rotation_delay_ms:u64, rotation_maximum_ms:u64, execution_sequence:u64. Session bytes are split across events `7105` and `7125` to preserve the existing 256-byte effect bound; revoke emits `7106`.

`provision.py --produce-owner-registration --work-dir PATH` consumes protected `human-evidence-input/owner-native.json`, custody key references, `custody-credit.bin`, `recovery-policy.json`, `recovery-guardians.json` and the LXIP `human-owner-result.json`. It uses the real `layerxctl read-state`/`submit` client, checks successful signed native receipts, asks the HTTPS receipt authority to verify and retain their authorized batches, and validates the output with the existing owner-registration contract. Preserve its exclusive `owner-native-run` directory for reconciliation after failure. Key bootstrap, real vault deposit/attestation and deployment of these inputs remain cluster prerequisites.

The hosted identity verifier reports `batch_included` for matching native committed snapshots with complete retained execution coverage. This does not satisfy Human's checkpoint-finalised key-policy or session gates; those refusals remain until authenticated checkpoint verification is implemented. No end-to-end success is implied by the registration JSON's structural validation.

`owner-native.json` has exactly `node_socket`, `network_id`, `owner_seed_file`, `pending_seed_file`, `sequencer_public_key`, `layerxctl`, `fee_limit`, `authority_url`, `authority_token_file`, and `authority_ca_file`. The producer requires owner-only regular input files (0600), a real local node socket, an absolute CLI path and HTTPS authority with a trusted CA. Key files contain custody-produced seed32 and must already match the node bootstrap identities. The LXIP result and recovery-policy file must agree exactly.

`recovery-guardians.json` contains `public_keys`, a unique list of real lowercase H32 guardian public keys. Its root is SHA-256 of `LX:HUMAN:RECOVERY:v1` followed by NUL, threshold u16be, guardian-count u16be, then sorted public-key32 values. The threshold must not exceed the guardian count. The real custody attestation producer supplies `custody-credit.bin`, bound to this owner account/key and the node's immutable custody profile.
