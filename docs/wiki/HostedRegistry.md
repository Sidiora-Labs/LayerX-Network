...
`platform-hosted-topology-check` loads `platform/hosted/registry/deployment.yaml` among its default manifests (`platform/hosted/tests/topology-check.sh:17-24, 81-88`; `platform/Makefile.inc:177-178`). Beta-cluster apply order is testnet, then gateway, then registry, then developer (`platform/hosted/tests/beta-cluster.sh:825, 859-863`). The StatefulSet selects nodes labelled `layerx.io/program-registry-boundary=v1` (`platform/hosted/registry/deployment.yaml:11`; `platform/hosted/tests/beta-cluster.sh:87, 323`).
...
Build workspaces are hostPath quota slots at `/var/lib/layerx-program-registry-builds`, cgroup at `/sys/fs/cgroup/layerx-program-registry` (`platform/hosted/registry/deployment.yaml:61-62, 70-71`). The node oneshot unit runs `node-provision-build-boundary.sh` with `LAYERX_REGISTRY_MAX_BUILDS=4`, `LAYERX_REGISTRY_BUILD_QUOTA_BYTES=5368709120`, `LAYERX_REGISTRY_BUILD_QUOTA_INODES=65536` (`platform/hosted/registry/layerx-program-registry-boundary.service:10-13`). The script creates UID 4030 cgroup delegation (`cpu memory pids io`) and per-slot ext4 loop filesystems (`platform/hosted/registry/node-provision-build-boundary.sh:15-73`). `HermeticBuilder` refuses a cgroup not owned by 4030:4030, missing controllers, or zero mounted quota slots (`platform/hosted/registry/src/builder.rs:380-421`). Each rebuild runs `layerx-cgroup-exec` then `bwrap --unshare-all --disable-userns --cap-drop ALL --ro-bind` of the pinned environment, no network, `CARGO_NET_OFFLINE=true` (`platform/hosted/registry/src/builder.rs:274-338`). Artifact open uses `openat2` with `ResolveFlags::BENEATH` (`platform/hosted/registry/src/builder.rs:698-713`). Artifact bound is 32 MiB (`platform/hosted/registry/src/builder.rs:25`).
...
| `LAYERX_REGISTRY_BUILDER_CGROUP_ROOT` | required (`platform/hosted/registry/src/main.rs:281-284`) | `/run/layerx/cgroup` |
...
| `main::tests::delegation_quota_and_open_inode_execution_fail_closed` | Provisioner and unit file contain cgroup/quota/capability strings; deployment contains `layerx.io/program-registry-boundary: "v1"`; builder contains `NonBlockingLockExclusive` and `/proc/self/fd/` (`platform/hosted/registry/src/main.rs:915-946`) |
