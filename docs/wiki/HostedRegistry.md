...
`layerx-program-registry` answers the developer CLI's registry routes from durable local evidence only: reads are re-verified against the canonical deployment journal, and verified-source status is produced by rebuilding published source in a pinned toolchain environment (`platform/hosted/registry/src/lib.rs:1-4`; `platform/hosted/registry/Cargo.toml:12-14`; `platform/hosted/registry/src/main.rs:1`). The crate is `layerx-platform-registry` (`platform/hosted/registry/Cargo.toml:1-2`). The graph anchor is `receipt-verified-program-registry-v1` (`platform/hosted/registry/src/lib.rs:76`). The image is `ghcr.io/sidiora-labs/layerx-program-registry:0.1.0`, UID/GID `4030`, command `layerx-program-registry` (`platform/hosted/registry/Dockerfile:5-12`; `platform/hosted/registry/deployment.yaml:36-38`). A second binary, `layerx-cgroup-exec`, supervises one isolated build process tree (`platform/hosted/registry/Cargo.toml:16-18`; `platform/hosted/registry/src/bin/layerx-cgroup-exec.rs:1`).

It registers protocol-verified program deployments, upgrades, lifecycle, value-account bindings, published interfaces, operator-mirrored source, and rebuild verdicts. `POST /__registry/deployments` accepts a canonical signed Programs deploy or upgrade activity as an octet-stream body, submits those exact bytes through `LAYERX_REGISTRY_LNI_SOCKET` when configured or through the authenticated `LAYERX_REGISTRY_NODE_ENDPOINT` boundary, and publishes only after the native proof and independent receipt-authority check succeed (`platform/hosted/registry/src/routes.rs:3-8, 272, 729-792`; `platform/hosted/registry/src/main.rs:414`). It does not ingest caller-supplied `record_hex` or `proof_hex` projections. An empty body, or a node that cannot produce a proof, remains `503` `deployment_proof_unavailable` (`platform/hosted/registry/src/routes.rs:748-757, 1126-1131`). The journal commit unit and Human `pairs/` export are documented on [Registry deployment journal](RegistryDeploymentJournal.md).

The in-cluster Service is `layerx-program-registry` in `layerx-testnet` on TCP `9420` (`platform/hosted/registry/deployment.yaml:104-107`). Callers that name that URL are the gateway (`platform/hosted/gateway/deployment.yaml:84-85`; `platform/hosted/gateway/src/main.rs:553-557, 1670-1690, 393-411, 2559-2565`) and testnet-control (`platform/hosted/testnet/deployment.yaml:95`; `platform/hosted/testnet/src/main.rs:843-846, 1102`). Ingress NetworkPolicy admits `app: layerx-gateway`, `app: layerx-testnet-control`, and `layerx-role: source-publication-operator` on `9420` (`platform/hosted/registry/deployment.yaml:115-119`). Egress NetworkPolicy admits `layerx-plane: trusted-boundary` on TCP `9443`, `9445` and `9446`, plus DNS `53` (`platform/hosted/registry/deployment.yaml:120-124`). A second policy admits the registry onto the node agent-boundary Service port `9443` (`platform/hosted/registry/deployment.yaml:126-134`). The node ingress policy admits the registry pod on authority and agent-boundary container ports `9445` and `9446` (`platform/hosted/node/deployment.yaml:850-851`). The registry env URLs use Service port `9443` on `layerx-agent-boundary` and `layerx-receipt-authority` (`platform/hosted/registry/deployment.yaml:65-68`; `platform/hosted/node/deployment.yaml:830-835`). Those Services map `9443` to container ports `9446` and `9445`.

`platform-hosted-topology-check` loads `platform/hosted/registry/deployment.yaml` among its default manifests (`platform/hosted/tests/topology-check.sh:17-24, 81-88`; `platform/Makefile.inc:177-178`). Beta-cluster apply order is registry journal and trusted-boundary manifests first, then Human, testnet, gateway, and registry (`platform/hosted/tests/beta-cluster.sh:1078-1101`). The StatefulSet selects nodes labelled `layerx.io/program-registry-boundary=v2` (`platform/hosted/registry/deployment.yaml:11`; `platform/hosted/tests/beta-cluster.sh:89`).
...
| --- | --- | --- | --- |
| GET | `/healthz` | mTLS only | `200` `{"status":"ready","service":"program-registry"}` (`platform/hosted/registry/src/routes.rs:268-271`) |
| POST | `/__registry/deployments` | request | Submit a canonical signed Programs deploy or upgrade activity; publish the verified envelope and Human `pairs/` export, or refuse (`platform/hosted/registry/src/routes.rs:272, 729-792`) |
| POST | `/__registry/head` | request | Refresh journal observed head from the independently verified current protocol head (`platform/hosted/registry/src/routes.rs:273, 836-858`) |
| POST | `/__registry/sources` | publication | Publish archive + plan + URI into the source mirror (`platform/hosted/registry/src/routes.rs:274, 861`) |
...
| 503 | `deployment_proof_unavailable` | Empty deployment body, or the node cannot produce a canonical proof (`platform/hosted/registry/src/routes.rs:748-757, 1126-1131`) |
| 503 | `journal_unavailable` | Journal load or envelope append failed (`platform/hosted/registry/src/routes.rs:732, 782`) |
| 503 | `journal_export_unavailable` | Human `pairs/` export failed after a verified unit (`platform/hosted/registry/src/routes.rs:744, 790`) |
| 503 | `clock_unavailable` | Deployment verification needs a valid clock (`platform/hosted/registry/src/routes.rs:764-768`) |
| 503 | `receipt_authority_unavailable` | Independent receipt-authority check failed (`platform/hosted/registry/src/routes.rs:775`) |
| 422 | `deployment_proof_refused` | Native proof or stored-unit replay refused (`platform/hosted/registry/src/routes.rs:741, 772`) |
| 422 | `deployment_projection_refused` | Verified evidence cannot be projected (`platform/hosted/registry/src/routes.rs:779`) |
...
| `LAYERX_REGISTRY_JOURNAL` | default `$STATE/journal` (`platform/hosted/registry/src/main.rs:413`) | `/var/lib/layerx-registry-journal` |
| `LAYERX_REGISTRY_LNI_SOCKET` | optional Unix socket; when set, deployment submit uses it instead of the node HTTPS boundary (`platform/hosted/registry/src/main.rs:414`) | unset |
...
| `routes::tests::deployment_ingress_stays_blocked_for_forgeable_record_shape` | The empty-body helper still returns `503` for a `record_hex` JSON body (`platform/hosted/registry/src/routes.rs:1180-1183`) |
| `routes::tests::deployment_ingress_does_not_accept_caller_proof_bytes` | The empty-body helper still returns `503` for a `proof_hex` JSON body (`platform/hosted/registry/src/routes.rs:1186-1189`) |
...
`builder.rs` and `node_state.rs` now carry unit tests. Native deployment submit and proof verification are exercised in `tests/native_deployment.rs`. There are no `#[test]` functions in `mirror.rs`, `verified.rs`, or `program_state.rs`.

---

## Facts the sources disagree on or leave unset

`POST /__registry/deployments` now writes the journal after native proof verification (`platform/hosted/registry/src/routes.rs:781-792`; `platform/hosted/registry/src/journal.rs:501-505`). `Registrar::rebuild` reloads envelopes and regenerates Human `pairs/` exports (`platform/hosted/registry/src/routes.rs:795-802`). The remaining host-side Human assembler still needs a protected local copy of `pairs/`; see [Registry deployment journal](RegistryDeploymentJournal.md).

NetworkPolicy ingress names `layerx-role: source-publication-operator` (`platform/hosted/registry/deployment.yaml:118-119`). No workload with that label is in the default topology-check manifests (`platform/hosted/tests/topology-check.sh:81-88`).

`GET /healthz` does not inspect protocol state (`platform/hosted/registry/src/routes.rs:268-271`). The kubelet probe does not call it (`platform/hosted/registry/deployment.yaml:74`; `platform/hosted/registry/src/main.rs:848-849`).
