...
`release_candidate` names the single revision the beta is qualified at. While it is `unset` the beta has no release candidate: `tools/ci/beta-report.sh` treats no gate record as evidence and renders the decision `no-go`, and `tools/ci/beta-ledger-check.sh` binds every gate revision to `HEAD` alone. Setting it to the 40-hex commit of the release candidate widens that binding to the candidate's history and makes gate records on the candidate evidence for the report. `tools/ci/beta-ledger-check.sh` fails when a gate record names a revision that is an ancestor of neither the release candidate nor `HEAD`: such a record was executed on a line of development that leads to neither revision, so it proves nothing about the tree being checked and must be re-executed before it counts.
...
| report_path | spec/layerx-beta/report.md |
| report_status | no-go |
...
| Paxeer custody boundary | Paxeer production network | the in-cluster paxd chain 125 node and its JSON-RPC boundary from platform/hosted/paxeer/deployment.yaml, reached through testnet_paxeer_url; platform/hosted/tests/beta-cluster.sh builds paxd from paxeer-network/ (Dockerfile.paxd-node), initialises the chain with the generated deployer address and runs platform/hosted/paxeer/deploy-contracts.sh bootstrap from the node genesis using the explicit immediate-beta profile and LayerXBetaTimelock with zero governance delay; standard deployments use LayerXTimelock with a minimum one-day delay | LAYERX_BETA_FOUNDRY_BIN (pinned forge and cast); the deployer, controller and governance keys are generated by the bring-up |
