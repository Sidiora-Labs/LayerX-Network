...
| gateway_url | https://api.paxeer.network | .github/workflows/platform.yml LAYERX_GATEWAY_URL; platform/hosted/testnet/src/lib.rs; platform/docs/beta-environment.md; platform/examples/*/layerx.example.json |
...
| report_path | build/qualification/beta-report.md |
| report_status | no-go |
...
The go/no-go report is local, ignored build output. Run `make beta-report` to generate it or `make beta-contract-check` to generate and validate it. The report at `report_path` is rendered from the evidence ledger by `report_generator`, which reads gate records alone: `report_status` is its decision line, and the reached rung of every surface above is the rung that report records. `tools/ci/beta-report.sh --check` fails when this contract and that report disagree, when the report on disk is not the one the ledger renders, or when readiness is claimed with a surface below its required rung.
...
| Paxeer custody boundary | Paxeer production network | the in-cluster paxd chain 125 node and its JSON-RPC boundary from platform/hosted/paxeer/deployment.yaml, reached through testnet_paxeer_url; platform/hosted/tests/beta-cluster.sh builds paxd from the repository-root Go module (Dockerfile.paxd-node), initialises the chain with the generated deployer address and the layerxcustody (0x…1013) and layerxanchor (0x…1014) module genesis written by platform/hosted/paxeer/custody-genesis.py and anchor-genesis.py, and runs platform/hosted/paxeer/anchor-guarantors.sh to register and activate the node's guarantors through the anchor precompile; no Solidity contract is deployed for custody, checkpoints, bonds or challenges | none; the deployer and controller keys are generated by the bring-up and Paxeer transactions are signed by platform/hosted/paxeer/evm.py (LAYERX_BETA_MIRROR_FOUNDRY_BIN is needed only for the Ethereum mirror archive contract) |
...
| --- | --- | --- | --- | --- |
| faucet_hostname | faucet.paxeer.network | platform/hosted/testnet/deployment.yaml: Service layerx-faucet-public is a LoadBalancer with no Ingress host | (no ingress host) | 3.7 |
| placeholder_hostname | paxeer.network | platform/hosted/webhooks/deployment.yaml Ingress layerx-developer and layerx-developer-web host | developers.layerx.example | 3.7 |
