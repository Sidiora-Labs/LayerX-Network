...
| Paxeer custody boundary | Paxeer production network | the in-cluster paxd chain 125 node and its JSON-RPC boundary from platform/hosted/paxeer/deployment.yaml, reached through testnet_paxeer_url; platform/hosted/tests/beta-cluster.sh builds paxd from the repository-root Go module (Dockerfile.paxd-node), initialises the chain with the generated deployer address and runs platform/hosted/paxeer/deploy-contracts.sh bootstrap from the node genesis using the explicit immediate-beta profile and LayerXBetaTimelock with zero governance delay; standard deployments use LayerXTimelock with a minimum one-day delay | LAYERX_BETA_FOUNDRY_BIN (pinned forge and cast); the deployer, controller and governance keys are generated by the bring-up |
