...
import { validateGrantDraw } from "../../../../agent/sdk/typescript/dist/src/x402/grant.js";
...
test("native signed receive matches subscription offer without authorizing fulfillment", () => {
  ...
  for (const change of [{ amount: "1" }, { asset: "ab".repeat(32) }, { payTo: "ab".repeat(32) }, { scheme: "metered" },
    { extra: { layerx: { ...offer.extra.layerx, payer: "ab".repeat(32) } } },
    { extra: { layerx: { ...offer.extra.layerx, purposeHash: "ab".repeat(32) } } }]) assert.throws(() => validateGrantDraw(wire, { ...offer, ...change }, r.idempotency_key, 7, 0n));
...
test("offer parsing requires explicit grant terms and rejects unknown schemes", () => {
  ...
  header(offer);
  for (const change of [{ scheme: "unknown" }, { extra: {} },
    { extra: { layerx: { ...offer.extra.layerx, payer: undefined } } },
    { extra: { layerx: { ...offer.extra.layerx, purposeHash: undefined } } },
    { extra: { layerx: { ...offer.extra.layerx, windowSeconds: "0" } } }]) assert.throws(() => header({ ...offer, ...change }));
});
...
test("buyer grant header preserves the selected terms and native idempotency key", async () => {
  ...
  for (const accepted of [
    { ...offer, amount: "1" },
    { ...offer, extra: { layerx: { ...offer.extra.layerx, payer: "ab".repeat(32) } } },
    { ...offer, extra: { layerx: { ...offer.extra.layerx, purposeHash: "ab".repeat(32) } } },
  ]) assert.throws(() => grantPaymentHeader(required, accepted, Buffer.from(wire).toString("hex")));
});
