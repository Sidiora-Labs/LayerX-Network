...
fn parse_request(stream: &mut TcpStream) -> Result<Request, String> {
    ...
    let (mut request, content_length) = parse_request_headers(&bytes[..header_end])?;
...
fn verified_program_document(
    ...
    let (terminal_payload, call_graph) = artifacts;
...
fn program_call(emulator: &mut Emulator, request: &Request, trace: u64) -> Response {
    ...
    execute_program_mutation(emulator, &decoded, head, lifecycle, trace)
}
...
fn move_quote(emulator: &mut Emulator, request: &Request, trace: u64) -> Response {
    ...
    resolve_move_quote(emulator, body, amount, &source_authority, trace)
}
...
fn move_commit(emulator: &mut Emulator, request: &Request, trace: u64) -> Response {
    let (idempotency, quote, state) = match prepare_move_commit(emulator, request, trace) {
        Ok(value) => value,
        Err(response) => return response,
    };
    let (source, destination) = match validate_move_accounts(emulator, &quote, trace) {
        Ok(value) => value,
        Err(response) => return response,
    };
    ...
    let (mut receipt, code) = match execute_move_activity(emulator, &activity, trace) {
        Ok(value) => value,
        Err(response) => return response,
    };
    ...
    let protocol = match bind_move_receipt(
        emulator,
        verified.receipt().protocol(),
        &material,
        &MoveVerification {
            quote: &quote,
    ...
    if let Err(response) =
        verify_move_post_state(emulator, &quote, &idempotency, &state, protocol, trace)
    {
        return response;
    }
    ...
    complete_move_response(
        emulator,
        &quote,
        idempotency,
        (&material, expected_activity_id, receipt_digest),
        (state.timestamp_ms, code),
        trace,
    )
}
...
fn canonical_program_response(response: &str, activity_id: &str, idempotency_key: &str) -> bool {
    ...
    document.is_object()
        && serde_json::to_string(&document).is_ok_and(|canonical| canonical == response)
        && document
            .get("activity_id")
            .and_then(serde_json::Value::as_str)
            == Some(activity_id)
...
fn encode_recovery_snapshot(
    ...
    encode_snapshot_receipts(&mut encoded, indexes)?;
    encode_snapshot_accounts(&mut encoded, indexes)?;
    encode_snapshot_moves(&mut encoded, indexes)?;
    let mut digest = Sha256::new();
...
fn import_snapshot(emulator: &mut Emulator, body: &[u8], trace: u64) -> Response {
    ...
    let previous_snapshot = unsafe { slice::from_raw_parts(prior_bytes, prior_length) }.to_vec();
    let code = unsafe {
...
fn program_registry_read(
    ...
    program_interface_response(&program, &live, valid_through, trace)
}
...
fn program_simulate(emulator: &mut Emulator, request: &Request, trace: u64) -> Response {
    ...
    let execution = match verified_program_document(
        &verified,
        (&material.terminal_payload, &material.call_graph),
        program_id,
        head.abi_version,
        emulator.signing_key.verifying_key().to_bytes(),
        "simulated",
...
fn platform_emulator(config: Config) -> Result<(), String> {
    ...
    let transition_receiver = start_http_workers(listener)?;
...
mod program_call_tests {
    use super::{
        ...
        MoveQuoteRecord, ProgramOperation, Request, SnapshotIndexes, MAX_RECEIPT_BYTES,
    };
    ...
    fn recovery_snapshot_is_deterministic_and_preserves_program_indexes() {
        ...
        let first = encode_recovery_snapshot(b"bounded-core-snapshot", indexes)
            .unwrap_or_else(|error| panic!("test operation failed: {error:?}"));
        let second = encode_recovery_snapshot(b"bounded-core-snapshot", indexes)
            .unwrap_or_else(|error| panic!("test operation failed: {error:?}"));
...
fn execute_program_mutation(
    ...
    finish_program_mutation(emulator, decoded, head, &before, lifecycle, receipt, trace)
}

fn finish_program_mutation(
    emulator: &mut Emulator,
    decoded: &DecodedProgramActivity,
    head: Option<CoreProgram>,
    before: &CoreState,
    lifecycle: bool,
    mut receipt: CoreReceipt,
    trace: u64,
) -> Response {
    let activity_id = hex_encode(&decoded.activity_id);
    let protocol_idempotency = hex_encode(&decoded.idempotency_key);
    ...
    finish_program_call(emulator, decoded, head, before, &material, trace)
}

fn resolve_move_quote(
    emulator: &mut Emulator,
    body: MoveQuoteBody,
    amount: u128,
    source_authority: &EmulatorAccount,
    trace: u64,
) -> Response {
...
type SignedActivity = (Vec<u8>, [u8; 32], [u8; 32]);

type SignedMove = (Vec<u8>, [u8; 32], [u8; 32], [u8; 32]);
...
fn parse_request_headers(header_bytes: &[u8]) -> Result<(Request, usize), String> {
    let headers = std::str::from_utf8(header_bytes).map_err(|_| "headers are not UTF-8")?;
...
fn finish_move_quote(
    emulator: &mut Emulator,
    body: MoveQuoteBody,
    amount: u128,
    accounts: (CoreAccountView, CoreAccountView),
    identity_sequence: u64,
    trace: u64,
) -> Response {
    ...
    let (Ok(source_length), Ok(destination_length)) = (
        u16::try_from(body.source.len()),
        u16::try_from(body.destination.len()),
    ) else {
        return refusal(
            trace,
            400,
...
) -> Result<std::sync::mpsc::Receiver<ParsedConnection>, String> {
...
type DecodedReceipts = (HashMap<String, String>, VecDeque<String>);

fn decode_snapshot_receipts(
    authenticated: &[u8],
    offset: &mut usize,
    receipt_count: usize,
) -> Result<DecodedReceipts, String> {
...
type DecodedPrograms = (HashMap<String, ProgramOperation>, HashMap<String, String>);

fn decode_snapshot_programs(
    authenticated: &[u8],
    offset: &mut usize,
    operation_count: usize,
) -> Result<DecodedPrograms, String> {
...
type DecodedAccounts = HashMap<String, EmulatorAccount>;
...
type DecodedQuotes = HashMap<String, MoveQuoteRecord>;
...
type DecodedMoves = HashMap<String, MoveOperation>;
...
fn prepare_move_commit(
    emulator: &Emulator,
    request: &Request,
    trace: u64,
) -> Result<(String, MoveQuoteRecord, CoreState), Response> {
    ...
    let body = match decode_json::<MoveCommitBody>(request) {
        Ok(value) => value,
        Err(error) => return Err(refusal(trace, 400, "invalid_move_commit", &error)),
    };
    if let Some(operation) = emulator.move_operations.get(&idempotency) {
        if operation.quote_id == body.quote_id {
            return Err(stored_move_operation(operation));
        }
        return Err(refusal(
            trace,
            409,
    ...
    Ok((idempotency, quote, state))
}

fn validate_move_accounts(
    emulator: &Emulator,
    quote: &MoveQuoteRecord,
    trace: u64,
) -> Result<(CoreAccountView, CoreAccountView), Response> {
...
fn bind_move_receipt<'a>(
    emulator: &mut Emulator,
    protocol: Option<&'a layerx_wire::receipt::ProtocolReceipt>,
    material: &OwnedCoreReceipt,
    context: &MoveVerification<'_>,
    trace: u64,
) -> Result<&'a layerx_wire::receipt::ProtocolReceipt, Response> {
    let MoveVerification {
        quote,
        idempotency,
        state,
        source,
...
struct MoveVerification<'a> {
    quote: &'a MoveQuoteRecord,
    idempotency: &'a str,
    state: &'a CoreState,
    source: &'a CoreAccountView,
    destination: &'a CoreAccountView,
    expected_activity_id: [u8; 32],
    expected_context_hash: [u8; 32],
    expected_authorization_hash: [u8; 32],
}

fn verify_move_post_state(
    emulator: &mut Emulator,
    quote: &MoveQuoteRecord,
    idempotency: &str,
    state: &CoreState,
...
fn complete_move_response(
    emulator: &mut Emulator,
    quote: &MoveQuoteRecord,
    idempotency: String,
    evidence: (&OwnedCoreReceipt, [u8; 32], [u8; 32]),
    outcome: (u64, c_int),
    trace: u64,
) -> Response {
    let (material, expected_activity_id, receipt_digest) = evidence;
    let (timestamp_ms, code) = outcome;
    remember_receipt(
        emulator,
...
fn execute_move_activity(
    emulator: &mut Emulator,
    activity: &[u8],
    trace: u64,
) -> Result<(CoreReceipt, c_int), Response> {
    let mut receipt = empty_core_receipt();
    let code = unsafe {
        platform_emulator_execute(
            emulator.core,
    ...
    if code != 0 && code != -904 {
        return Err(core_response(trace, code));
    }
    Ok((receipt, code))
}
