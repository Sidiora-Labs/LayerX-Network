...
- `agent`: `HUMAN_PEERS`, `HUMAN_LIMIT_SCOPE`, `HUMAN_LIMIT_SCOPE_ID`, `HUMAN_LIMIT_ID`, `HUMAN_LIMIT_NAME`, `HUMAN_LIMIT_CEILING`, `HUMAN_LIMIT_CONSUMED`, `PROGRAM_PROBE_ID`. The sole peer entry is `uid=4020;tenant=<tenant>;principal=<principal>` as parsed by agentd. Limits must reflect the authority's policy and consumed state; IDs use agentd's exact hexadecimal widths. The Programs probe must be genuinely deployed.
...
The Human Dockerfile builds the service, components, Human LXKP KMS, agentd and the three provider binaries. The manifest uses their agreed binary, environment and probe interfaces.
