...
fn bytes(value: &str, maximum: usize) -> Result<Vec<u8>, String> {
    ...
    super::decode_hex(value, maximum)
}
...
mod tests {
    ...
    #[test]
    fn native_transport_retains_original_signed_bytes() -> Result<(), Box<dyn std::error::Error>> {
        let (body, registry) = fixture()?;
        let encoded = serde_json::to_vec(&body)?;
        ...
        for (content_type, body) in [
            ...
            let request = super::super::IncomingRequest {
            ...
            let decoded = super::super::program_call_bytes(&request, &registry)?;
            assert_eq!(decoded.0, signed);
            assert_eq!(decoded.1, program_id);
        }
        ...
        assert!(super::super::program_call_bytes(&request, &registry).is_err());
        Ok(())
    }
}
