...
url, deployment_path, beneficiary, amount_text, output = sys.argv[1:]
parsed = urlsplit(url)
...
require(amount > 0, "positive recipient funding required")
deployment = json.loads(Path(deployment_path).read_text())
account = rpc.call("eth_accounts", [])[0]
send(rpc, account, deployment["token"], calldata("deposit()"), amount)
send(rpc, account, deployment["token"], calldata("approve(address,uint256)", deployment["vault"], amount))
receipt = send(rpc, account, deployment["vault"], calldata("deposit(bytes32,uint256,bytes32)", deployment["asset"], amount, beneficiary))
