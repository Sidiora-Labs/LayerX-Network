...
impl Client {
    ...
    pub fn request_authorized_traced(
        ...
        self.request_with_principal(endpoint, authorization, request, trace, None)
    }

    /// # Errors
    /// Refuses invalid publication keys or any failure of the existing TLS boundary.
    pub fn request_with_principal(
        &self,
        endpoint: &Endpoint,
        authorization: &str,
        request: &OutboundRequest<'_>,
        ...
        if publication_key.is_some_and(|key| {
            key.is_empty() || key.len() > 4096 || key.bytes().any(|byte| byte.is_ascii_control())
        }) {
            return Err("invalid publication key".to_owned());
        }
        let OutboundRequest {
            method,
            path,
            ...
            .take(8)
        {
            match TcpStream::connect_timeout(&address, CONNECT_TIMEOUT) {
                Ok(tcp) => {
                    ...
                    let principal = publication_key
                        .map_or_else(String::new, |key| format!("LayerX-Key: {key}\r\n"));
                    write!(
                        stream,
                        "{method} {}{path} HTTP/1.1\r\nHost: {}\r\nAuthorization: {authorization}\r\nAccept: application/json\r\nContent-Type: {content_type}\r\n{idempotency}{trace}{principal}Content-Length: {}\r\nConnection: close\r\n\r\n",
                        endpoint.base_path,
                        endpoint.authority(),
                        body.len()
                    )
