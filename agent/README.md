...
Payment encodings and the 18-tool catalogue containing `wallet.*` and
`token.*` operations are on the testnet branch. They use the same daemon
prepare, disclose, sign, submit, and track path as other writes. Developer path:
