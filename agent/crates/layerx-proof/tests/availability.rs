...
fn chunks() -> (Vec<Chunk>, Vec<layerx_proof::merkle::Proof>, [u8; 32]) {
    ...
    let chunks = sections.into_iter().enumerate().map(|(index, (class, bytes))| Chunk {
        batch_number: 7,
...
fn authenticate(mut chunks: Vec<Chunk>) -> (Vec<Chunk>, Vec<layerx_proof::merkle::Proof>, [u8; 32]) {
    for chunk in &mut chunks {
        chunk.claimed_hash = availability_chunk_digest(
            chunk.batch_number, chunk.index, chunk.class as u8, chunk.class_offset, &chunk.bytes,
...
fn verified(chunks: Vec<Chunk>) -> Vec<VerifiedChunk> {
    ...
    chunks.into_iter().zip(proofs).map(|(chunk, proof)| {
...
fn unrelated_caller_records_cannot_authenticate_unrelated_chunk_bytes() {
    ...
    let failure = verify_reassembled(&material, &records, commitments)
        .err().unwrap_or_else(|| panic!("the former five-byte positive must fail canonical decoding"));
    ...
    let supplied = ReassembledRecords { activities: &substituted, ..records.clone() };
    let mut substituted_roots = commitments;
    substituted_roots.activity = root(&substituted).unwrap_or_else(|error| panic!("record root: {error:?}"));
    let failure = verify_reassembled(&material, &supplied, substituted_roots)
        .err().unwrap_or_else(|| panic!("self-consistent caller records and roots are unrelated to authenticated bytes"));
    ...
    let failure = verify_reassembled(&material, &supplied, commitments)
        .err().unwrap_or_else(|| panic!("supplied records must equal decoded records even with genuine roots"));
...
fn all_classes_do_not_prove_withheld_state_or_recovery_suffixes() {
    for class in [AvailabilityClass::StateDiff, AvailabilityClass::Recovery] {
        let (mut chunks, _, _) = chunks();
        let first = chunks.iter().position(|chunk| chunk.class == class).unwrap_or_else(|| panic!("class absent"));
        ...
        let failure = verify_reassembled(&material, &records, commitments)
            .err().unwrap_or_else(|| panic!("all five prefixes cannot prove full availability"));
...
fn duplicate_reordered_cross_root_and_cross_batch_material_is_refused() {
    ...
    assert_eq!(verify_reassembled(&duplicate, &records, commitments).err().unwrap_or_else(|| panic!("duplicate")).check,
    ...
    assert_eq!(verify_reassembled(&mixed, &records, commitments).err().unwrap_or_else(|| panic!("cross root")).check,
        AvailabilityCheck::ChunkOrder);
    let mut cross_batch_chunks = chunks;
    for chunk in &mut cross_batch_chunks { chunk.batch_number = 8; }
    let different_batch = verified(cross_batch_chunks);
    mixed[4] = different_batch[4].clone();
    assert_eq!(verify_reassembled(&mixed, &records, commitments).err().unwrap_or_else(|| panic!("cross batch")).check,
        AvailabilityCheck::ChunkOrder);
}
...
fn authenticated_noncanonical_class_order_offsets_and_empty_chunks_are_refused() {
    ...
    assert_eq!(verify_reassembled(&verified(wrong_order), &records, commitments)
        .err().unwrap_or_else(|| panic!("class order")).check, AvailabilityCheck::ChunkOrder);
    let mut wrong_offset = chunks.clone();
    wrong_offset[3].class_offset = 1;
    assert_eq!(verify_reassembled(&verified(wrong_offset), &records, commitments)
        .err().unwrap_or_else(|| panic!("class offset")).check, AvailabilityCheck::ClassOffset);
    ...
    extra.bytes.clear();
    empty.push(extra);
    assert_eq!(verify_reassembled(&verified(empty), &records, commitments)
        .err().unwrap_or_else(|| panic!("empty tail")).check, AvailabilityCheck::ClassOffset);
}
...
fn canonical_record_count_lengths_tags_and_trailing_bytes_are_enforced() {
    ...
    for bytes in [chunks[0].bytes[4..].to_vec(), wrong_count, trailing, excessive,
        chunks[0].bytes[..chunks[0].bytes.len()-1].to_vec()] {
        let mut altered = chunks.clone();
        altered[0].bytes = bytes;
        assert_eq!(verify_reassembled(&verified(altered), &records, commitments)
            .err().unwrap_or_else(|| panic!("canonical record encoding")).check, AvailabilityCheck::RecordEncoding);
    }
    for bytes in [tagged(0, b"receipt"), tagged(3, b"receipt"),
        [tagged(2, b"event"), tagged(1, b"receipt")].concat()] {
        let mut altered = chunks.clone();
        altered[1].bytes = bytes;
        assert_eq!(verify_reassembled(&verified(altered), &records, commitments)
            .err().unwrap_or_else(|| panic!("canonical receipt tags")).check, AvailabilityCheck::RecordEncoding);
    }
}
...
fn chunk_size_offset_and_leaf_count_bounds_match_native_verification() {
    ...
    assert_eq!(verify_chunk(oversized, &proofs[0], 7, &commitment)
        .err().unwrap_or_else(|| panic!("chunk size")).check, AvailabilityCheck::ChunkBounds);
    let mut overflow = chunks[0].clone();
    overflow.class_offset = u64::MAX;
    assert_eq!(verify_chunk(overflow, &proofs[0], 7, &commitment)
        .err().unwrap_or_else(|| panic!("offset overflow")).check, AvailabilityCheck::ChunkBounds);
    let proof = layerx_proof::merkle::Proof::new(0, 4097, vec![[0; 32]; 13]).unwrap_or_else(|error| panic!("proof shape: {error:?}"));
    assert_eq!(verify_chunk(chunks[0].clone(), &proof, 7, &commitment)
        .err().unwrap_or_else(|| panic!("chunk count")).check, AvailabilityCheck::ChunkBounds);
}
...
fn individually_valid_proofs_cannot_disagree_about_bundle_leaf_count() {
    ...
    let alternate = layerx_proof::merkle::Proof::new(
        0, 6, proofs[0].siblings().to_vec(),
    ).unwrap_or_else(|error| panic!("same-depth alternate count: {error:?}"));
    material[0] = verify_chunk(chunks[0].clone(), &alternate, 7, &commitment)
        .unwrap_or_else(|error| panic!("this interior leaf path alone cannot establish total leaf count: {error:?}"));
    let (records, commitments) = records();
    assert_eq!(verify_reassembled(&material, &records, commitments)
        .err().unwrap_or_else(|| panic!("mixed proof counts")).check, AvailabilityCheck::BundleCompleteness);
}
