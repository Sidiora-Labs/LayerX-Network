...
pub(crate) fn system(endpoint: &str) -> Option<TlsConfig> {
    ...
    if endpoint.starts_with("https://") {
        ...
        certificates.extend(loaded.certs.iter().map(|certificate| {
            Certificate::from_der(certificate.as_ref()).to_owned()
        }));
    }
    Some(TlsConfig::builder().provider(TlsProvider::Rustls)
        .root_certs(RootCerts::new_with_certs(&certificates)).build())
}
...
fn private_ca_checks_the_actual_server_identity() {
    tls_boundary::qualify("outbound_tls::private_ca_checks_the_actual_server_identity", |endpoint| {
        ...
        let agent: ureq::Agent = ureq::Agent::config_builder().tls_config(tls)
            .timeout_global(Some(std::time::Duration::from_secs(5))).build().into();
        agent.get(format!("{endpoint}/livez")).call().map_err(|error| error.to_string())?
            .body_mut().read_to_vec().map_err(|error| error.to_string())
    });
}
