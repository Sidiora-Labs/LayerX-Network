use super::{decode_batch_header, verify_receipt_inclusion, AuthorizedBatch, RawReceiptEvidence,
    ReceiptEvidenceError, SequencerAuthorization, VerifiedReceiptEvidence, VerifierPolicyError};
use layerx_proof::receipt::{authorized_maintained_activity_batch_chain, verify_native_owner_outcome,
...
impl VerifiedReceiptEvidence {
    pub(crate) fn verify_authorized_native_owner(
        ...
        let authorization = SequencerAuthorization::new(header.sequencer_id(),
            batch.sequencer_public_key(), header.batch_number(), header.batch_number());
        let inclusion = verify_receipt_inclusion(raw.canonical_receipt(), raw.proof(),
            raw.canonical_header(), &raw.header_signature(), &authorization)
        ...
        let authenticated = if let Some((evidence, receipts)) = maintained {
            ...
            let sealed = AuthorizedBatch::new(batch.batch_id(), batch.asset(),
                header.previous_state_root(), header.resulting_state_root(), batch.sequencer_public_key());
            authorized_maintained_activity_batch_chain(raw.canonical_receipt(), &sealed, evidence, receipts)
        ...
        if authenticated != *batch { return Err(ReceiptEvidenceError::BatchIdentity); }
        let verified = verify_native_owner_outcome(raw.canonical_receipt(), batch, expected)
            .map_err(ReceiptEvidenceError::NativeOwner)?;
        let protocol = verified.receipt().protocol().ok_or(ReceiptEvidenceError::BatchIdentity)?;
        ...
        if maintained.is_none() && receipt_execution_batch_id(protocol, &header)
            .map_err(|_| ReceiptEvidenceError::BatchIdentity)? != batch.batch_id()
        {
            return Err(ReceiptEvidenceError::BatchIdentity);
        }
        Ok(Self {
            receipt_ref: Sha256::digest(verified.canonical_bytes()).into(),
            activity_id: protocol.activity_id(), global_sequence: protocol.global_sequence(),
            result_code: protocol.result_code(), amount: protocol.amount(),
            module_id: protocol.module_id(), operation: protocol.operation(), verified, inclusion,
        })
    }
}
