...
fn withheld_recovery_tail_retains_evidence_without_emitting_complete_replay() {
    ...
    tail.claimed_hash = availability_chunk_digest(7, 5, tail.class as u8,
        tail.class_offset, &tail.bytes).unwrap_or_else(|error| panic!("tail digest: {error:?}"));
    ...
    let server = provider(&socket, fixture.clone(), 5, None);
    let gate = ConnectionGate::new(1);
    let mut transport = Uds::connect(&socket.0, &gate, transport_limits()).unwrap_or_else(|error| panic!("transport: {error:?}"));
    let mut providers = ProviderSet::new(vec![Provider {
        name: "recovery-tail".to_owned(), transport: &mut transport,
    }]);
    let root = std::env::temp_dir().join(format!("layerx-availability-tail-{}", std::process::id()));
    let mut store = Store::open(&root).unwrap_or_else(|error| panic!("store: {error:?}"));
    let mut audit = AvailabilityAudit::default();
    let outcome = availability(&mut store, &tenant(), &mut audit, &mut providers,
        &AvailabilityRequest {
            selector: AvailabilitySelector::Batch(7), checkpoint_id: [0x77; 32],
            context: context(&fixture, 100),
        }, |_| {}).unwrap_or_else(|error| panic!("availability: {error:?}"));
    let AvailabilityRead::Partial { failures, .. } = outcome else {
    ...
    assert_eq!(failures[0].mismatching_commitment, fixture.availability_root);
    assert_eq!(failures[0].served_bytes,
        fixture.chunks[..5].iter().flat_map(|chunk| chunk.bytes.iter().copied()).collect::<Vec<_>>());
    ...
    assert_eq!(recovered.list_object_ids(&tenant(), ObjectKind::Audit).len(), 1);
    drop(recovered);
    let _ = fs::remove_dir_all(root);
}
