...
const KEY: [u8; 32] = [0x17, 0xcb, 0x79, 0xfb, 0x2b, 0x41, 0x20, 0xf2, 0xb1, 0xec, 0x65, 0xe4, 0x19, 0x8d, 0x6e, 0x08, 0xb2, 0x8e, 0x81, 0x3f, 0xeb, 0x01, 0xe4, 0xa4, 0x00, 0x83, 0x9b, 0x85, 0xe1, 0x80, 0x80, 0xce];

fn verified(bytes: &[u8]) -> bool {
    let Ok(receipt) = decode(bytes) else { return false; };
    let Receipt::Protocol(protocol) = &receipt else { return false; };
    let Some(signature) = protocol.sequencer_signature() else { return false; };
    let Ok(unsigned) = encode_unsigned(&receipt) else { return false; };
...
fn native_supply_receipt_roundtrip_and_authenticated_tampering() {
    ...
    let Receipt::Protocol(protocol) = receipt else { panic!("protocol receipt required"); };
    ...
    for length in 0..NATIVE.len() { assert!(!verified(&NATIVE[..length])); }
