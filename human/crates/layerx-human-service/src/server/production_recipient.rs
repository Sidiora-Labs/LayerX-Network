...
impl RecipientServer {
    ...
    pub fn bind(components: Arc<ProductionComponents>, config: RecipientServerConfig) -> io::Result<Self> {
        let parent = config.socket.parent().ok_or_else(invalid)?;
        let info = fs::symlink_metadata(parent)?;
        if !config.socket.is_absolute() || fs::canonicalize(parent)? != parent
            || !info.is_dir() || info.uid() != rustix::process::geteuid().as_raw()
            || info.gid() != rustix::process::getegid().as_raw() || info.mode() & 0o027 != 0
            || config.caller_uid == 0 || config.caller_uid == rustix::process::geteuid().as_raw()
            || config.deadline.is_zero() || config.deadline > Duration::from_secs(30)
        { return Err(invalid()); }
        prepare_socket(&config.socket)?;
        let socket = net::socket_with(AddressFamily::UNIX, SocketType::SEQPACKET, SocketFlags::CLOEXEC, None)?;
        ...
        net::listen(&socket, 4)?;
        net::sockopt::set_socket_timeout(&socket, net::sockopt::Timeout::Recv, Some(Duration::from_millis(100)))?;
        Ok(Self { socket, config, identity: (info.dev(), info.ino()), components })
    }
...
impl ProductionComponents {
    fn sign_recipient(&self, request: Request) -> Result<serde_json::Value, ApiFailure> {
        ...
        let recipient = self.custody.evm_wallet(&principal, &key).map_err(|_| ApiFailure::forbidden())?;
...
fn prepare_socket(path: &std::path::Path) -> io::Result<()> {
    ...
    if !info.file_type().is_socket() || info.uid() != rustix::process::geteuid().as_raw()
        || info.gid() != rustix::process::getegid().as_raw() || info.mode() & 0o777 != 0o660 {
        return Err(invalid());
    }
    let probe = net::socket_with(AddressFamily::UNIX, SocketType::SEQPACKET,
    ...
    if net::connect(&probe, &SocketAddrUnix::new(path)?) != Err(rustix::io::Errno::CONNREFUSED) {
        return Err(io::Error::new(io::ErrorKind::AddrInUse, "recipient socket is live"));
    }
    let current = fs::symlink_metadata(path)?;
    if (current.dev(), current.ino()) != (info.dev(), info.ino()) { return Err(invalid()); }
    fs::remove_file(path)
}

fn invalid() -> io::Error { io::Error::new(io::ErrorKind::InvalidData, "recipient authority refused") }
