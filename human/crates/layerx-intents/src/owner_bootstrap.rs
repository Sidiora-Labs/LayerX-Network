...
impl NativeOwnerBootstrap {
    ...
    pub fn compile(&self, registry: &ModuleRegistry) -> Result<CompiledIntent, CompileError> {
        let (did, ordinal, count, key, numbers) = match self {
            ...
            Self::RecoveryPolicy {
                ...
                (
                    ...
                    5,
                    root.bytes(),
                    vec![u64::from(threshold.value()), *minimum_delay, *maximum_delay],
                )
            }
        };
        if key == [0; 32] {
        ...
        let did = hash::did_id_for_protocol(did, 3)
        ...
        for (index, number) in numbers.iter().enumerate() {
            if ordinal == 3 && index == 0 {
                wire(
                    ...
                    encoder.u16(u16::try_from(*number).map_err(|_| native_invalid())?),
                )?;
            } else {
        ...
        if decoder.u16().map_err(invalid)? != tag
            ...
            || decoder.fixed(32).map_err(invalid)? != did
            || decoder.fixed(32).map_err(invalid)? != key
        {
            return Err(native_invalid());
        }
        for (index, number) in numbers.iter().enumerate() {
