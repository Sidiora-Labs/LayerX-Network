...
impl NativeBudgetCreate {
    ...
    pub fn payload(&self) -> Result<Vec<u8>, IntentError> {
        ...
        if self.budget_id == [0; 32]
            ...
            || self.expiry_ms <= self.period_start_ms
            || !(1..=2).contains(&self.rollover)
        ...
        encoded
            .fixed(
                &crate::canonical::account_id_for_protocol(&self.budget_account, 3)
        ...
        encoded
            .u64(self.revocation_sequence)
            .map_err(invalid_wire)?;
        encoded.u8(self.rollover).map_err(invalid_wire)?;
        encoded
            .fixed(
                &crate::canonical::account_id_for_protocol(&self.source_account, 3)
                    .map_err(invalid_wire)?,
            )
            .map_err(invalid_wire)?;
