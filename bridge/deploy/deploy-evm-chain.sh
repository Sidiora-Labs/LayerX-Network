...
confirmed_chain_id=$(cast chain-id --rpc-url "$rpc") || fail "$rpc_variable did not answer eth_chainId"
...
if [ ! -r "$run" ]; then
    run="$EVM_ROOT/broadcast/DeployPaxeerXVault.s.sol/$chain_id/run-latest.json"
fi
[ -r "$run" ] || {
    cat "$work/deploy.log" >&2
    fail "the deployment wrote no broadcast record under $work/broadcast or $EVM_ROOT/broadcast"
}
...
deployer=$(cast wallet address --private-key "$deploy_key") || fail "$key_variable is not a deploy key"

normalise() { sed -e 's/ \[[^]]*\]//g' -e 's/[][]//g' -e 's/, */,/g'; }

deployed_code=$(cast code "$vault" --rpc-url "$rpc") || fail "the deployed code of $vault is unreadable"
...
deployed_owner=$(cast call "$vault" 'owner()(address)' --rpc-url "$rpc" | normalise)
deployed_threshold=$(cast call "$vault" 'threshold()(uint256)' --rpc-url "$rpc" | normalise)
[ "$deployed_threshold" = "$threshold" ] \
    || fail "$vault carries the threshold $deployed_threshold, and $config names $threshold"
deployed_attestors=$(cast call "$vault" 'attestors()(address[])' --rpc-url "$rpc" | normalise)
...
for index in "${!asset_addresses[@]}"; do
    ...
    mapfile -t reported < <(cast call "$vault" 'caps(address)(uint256,uint256)' "$asset" --rpc-url "$rpc" | normalise)
