...
mod candidate_raw {
    ...
    unsafe extern "C" {
        ...
        pub(super) fn storage_scan_scoped(
            selector: i32,
            prefix_pointer: i32,
            prefix_length: i32,
            cursor_pointer: i32,
        ...
        pub(super) fn signature_recover(
            message_pointer: i32,
            message_length: i32,
            signature_pointer: i32,
            signature_length: i32,
        ...
        pub(super) fn bigint_modexp_256(
            base_pointer: i32,
            base_length: i32,
            exponent_pointer: i32,
            exponent_length: i32,
            modulus_pointer: i32,
            modulus_length: i32,
            output_pointer: i32,
...
pub(crate) fn context_read(field: i32, output: &mut [u8]) -> Result<i32, ProgramError> {
    let status =
        unsafe { candidate_raw::context_read(field, pointer_mut(output)?, length(output)?) };
    ProgramError::from_status(status)
}

pub(crate) fn balance_read(
    account: &[u8; 32],
    asset: &[u8; 32],
    output: &mut [u8; 16],
) -> Result<i32, ProgramError> {
    let status = unsafe {
        candidate_raw::balance_read(
            pointer(account)?,
            32,
            pointer(asset)?,
            32,
...
pub(crate) fn signature_verify(
    ...
    let status = unsafe {
        candidate_raw::signature_verify(
            algorithm,
            pointer(message)?,
            length(message)?,
            pointer(public_key)?,
...
pub(crate) fn signature_recover(
    message: &[u8; 32],
    signature: &[u8; 64],
    recovery_id: i32,
    output: &mut [u8; 65],
    ...
    let status = unsafe {
        candidate_raw::signature_recover(
            pointer(message)?,
            32,
            pointer(signature)?,
            64,
...
pub(crate) fn bigint_binary(
    operation: unsafe extern "C" fn(i32, i32, i32, i32, i32, i32) -> i32,
    left: &[u8; 32],
    right: &[u8; 32],
    output: &mut [u8; 32],
) -> Result<i32, ProgramError> {
...
pub(crate) fn bigint_mul(
    left: &[u8; 32],
    right: &[u8; 32],
    output: &mut [u8; 64],
) -> Result<i32, ProgramError> {
    let status = unsafe {
        candidate_raw::bigint_mul_256(
            pointer(left)?,
            32,
            pointer(right)?,
            32,
...
pub(crate) fn bigint_div(
    left: &[u8; 32],
    right: &[u8; 32],
    output: &mut [u8; 32],
) -> Result<i32, ProgramError> {
    bigint_binary(candidate_raw::bigint_div_256, left, right, output)
}
pub(crate) fn bigint_rem(
    left: &[u8; 32],
    right: &[u8; 32],
    output: &mut [u8; 32],
) -> Result<i32, ProgramError> {
    bigint_binary(candidate_raw::bigint_rem_256, left, right, output)
}
pub(crate) fn bigint_modexp(
    base: &[u8; 32],
    exponent: &[u8; 32],
    modulus: &[u8; 32],
    output: &mut [u8; 32],
    ...
    let status = unsafe {
        candidate_raw::bigint_modexp_256(
            pointer(base)?,
            32,
            pointer(exponent)?,
            32,
...
pub(crate) fn storage_scan(
    ...
    let status = unsafe {
        candidate_raw::storage_scan_scoped(
            selector,
            pointer(prefix)?,
            length(prefix)?,
            pointer(cursor)?,
            length(cursor)?,
            i32::try_from(max_entries)
                .map_err(|_| ProgramError::value(Field::Buffer, Reason::TooLarge))?,
            i32::try_from(max_bytes)
                .map_err(|_| ProgramError::value(Field::Buffer, Reason::TooLarge))?,
            pointer_mut(output)?,
            length(output)?,
        )
    };
    let length = usize::try_from(ProgramError::from_status(status)?)
...
pub(crate) fn storage_drop_principal() -> Result<(), ProgramError> {
    storage_drop(STORAGE_SELECTOR_PRINCIPAL)
}
pub(crate) fn storage_drop_shared() -> Result<(), ProgramError> {
    storage_drop(STORAGE_SELECTOR_SHARED)
}
pub(crate) fn storage_scan_principal(
    prefix: &[u8],
    cursor: &[u8],
    max_entries: u32,
    max_bytes: u32,
...
pub(crate) fn storage_scan_shared(
    prefix: &[u8],
    cursor: &[u8],
    max_entries: u32,
    max_bytes: u32,
