...
impl<'a> ScanCursor<'a> {
    /// Constructs an opaque bounded cursor.
    ///
    /// # Errors
    ///
    /// Refuses an oversized cursor.
    pub const fn new(bytes: &'a [u8]) -> Result<Self, ProgramError> {
        if bytes.len() > MAX_SCAN_CURSOR_BYTES {
...
impl ScanLimits {
    /// Constructs scan limits.
    ///
    /// # Errors
    ///
    /// Refuses zero or protocol-widening limits.
    pub const fn new(max_entries: u32, max_bytes: u32) -> Result<Self, ProgramError> {
        let limits = Self {
            max_entries,
            max_bytes,
        };
        if limits.max_entries == 0
            || limits.max_entries > MAX_SCAN_ENTRIES
            || limits.max_bytes < 5
            || limits.max_bytes > 67_126_228
        {
            Err(ProgramError::value(Field::Buffer, Reason::TooLarge))
        } else {
            Ok(limits)
...
impl<'a> ScanPage<'a> {
    ...
    fn decode(encoded: &'a [u8], prefix: &[u8], limits: ScanLimits) -> Result<Self, ProgramError> {
        ...
        if encoded.len() > usize::try_from(limits.max_bytes).map_err(|_| malformed())? {
            return Err(malformed());
        }
        let mut decoder = PageDecoder::new(encoded);
        let count = decoder.u16()?;
        if u32::from(count) > limits.max_entries || u32::from(count) > MAX_SCAN_ENTRIES {
            return Err(malformed());
        }
        let mut previous: Option<&[u8]> = None;
        for _ in 0..count {
            let key_len = usize::from(decoder.u16()?);
            if key_len == 0 || key_len > MAX_STORAGE_KEY_BYTES {
                return Err(malformed());
            }
            let key = decoder.take(key_len)?;
            if !key.starts_with(prefix) || previous.is_some_and(|prior| prior >= key) {
                return Err(malformed());
            }
            previous = Some(key);
            let value_len = usize::try_from(decoder.u32()?).map_err(|_| malformed())?;
            if value_len > MAX_STORAGE_VALUE_BYTES {
                return Err(malformed());
            }
            decoder.take(value_len)?;
        }
        let entries_end = decoder.offset;
        let present = decoder.byte()?;
        if present > 1 {
            return Err(malformed());
        }
        let cursor_len = usize::from(decoder.u16()?);
        if cursor_len > MAX_SCAN_CURSOR_BYTES {
            return Err(malformed());
        }
        let cursor_start = decoder.offset;
        decoder.take(cursor_len)?;
        let cursor_end = decoder.offset;
        if !decoder.is_empty()
            || (present == 0 && cursor_len != 0)
            || (present == 1 && cursor_len == 0)
        {
            return Err(malformed());
        }
        Ok(Self {
    ...
    #[must_use]
    pub fn entries(&self) -> ScanEntries<'a> {
        ScanEntries {
            remaining: &self.encoded[2..self.entries_end],
        }
    }
    /// Returns the continuation when more entries remain.
    #[must_use]
    pub fn cursor(&self) -> Option<ScanCursor<'a>> {
        if self.cursor_start == self.cursor_end {
            None
        } else {
            Some(ScanCursor(
                &self.encoded[self.cursor_start..self.cursor_end],
            ))
        }
    }
}
...
impl<'a> PageDecoder<'a> {
    ...
    fn u16(&mut self) -> Result<u16, ProgramError> {
        Ok(u16::from_be_bytes(self.take(2)?.try_into().map_err(
            |_| ProgramError::value(Field::Buffer, Reason::Malformed),
        )?))
    }
...
impl<'a> Iterator for ScanEntries<'a> {
    type Item = ScanEntry<'a>;
    fn next(&mut self) -> Option<Self::Item> {
        if self.remaining.is_empty() {
            return None;
        }
        let key_len = usize::from(u16::from_be_bytes(self.remaining[..2].try_into().ok()?));
        let key = &self.remaining[2..2 + key_len];
        let value_offset = 2 + key_len;
        let value_len = usize::try_from(u32::from_be_bytes(
            self.remaining[value_offset..value_offset + 4]
                .try_into()
                .ok()?,
        ))
        .ok()?;
        let value = &self.remaining[value_offset + 4..value_offset + 4 + value_len];
        self.remaining = &self.remaining[value_offset + 4 + value_len..];
        Some(ScanEntry { key, value })
    }
}

#[cfg(target_arch = "wasm32")]
fn scan_with<'a>(
    host_call: fn(&[u8], &[u8], u32, u32, &mut [u8]) -> Result<usize, ProgramError>,
    prefix: &[u8],
    cursor: &ScanCursor<'_>,
    limits: ScanLimits,
    output: &'a mut [u8],
) -> Result<ScanPage<'a>, ProgramError> {
    if prefix.len() > MAX_STORAGE_KEY_BYTES
        || usize::try_from(limits.max_bytes)
            .map_err(|_| ProgramError::value(Field::Buffer, Reason::TooLarge))?
            > output.len()
    {
        return Err(ProgramError::value(Field::Buffer, Reason::TooLarge));
    }
    let length = host_call(
        prefix,
        cursor.bytes(),
        limits.max_entries,
        limits.max_bytes,
    ...
    ScanPage::decode(&output[..length], prefix, limits)
}

/// Scans principal-scoped storage.
///
/// # Errors
///
/// Refuses authority, bounds, meter, or malformed host output.
#[cfg(target_arch = "wasm32")]
pub fn scan<'a>(
    prefix: &[u8],
    cursor: &ScanCursor<'_>,
    ...
    scan_with(host::storage_scan_principal, prefix, cursor, limits, output)
}
/// Drops the complete principal-scoped namespace.
///
/// # Errors
///
/// Refuses missing authority or meter.
#[cfg(target_arch = "wasm32")]
pub fn drop_namespace() -> Result<(), ProgramError> {
    host::storage_drop_principal()
}
...
pub mod shared {
    #[cfg(target_arch = "wasm32")]
    use super::{
        decode_read_length, host, scan_with, Bytes, ScanCursor, ScanLimits, ScanPage, StorageValue,
    };
    use super::{Field, ProgramError, Reason, MAX_STORAGE_KEY_BYTES};
    ...
    /// every meter refusal.
    #[cfg(target_arch = "wasm32")]
    pub fn write(key: SharedStorageKey<'_>, value: StorageValue<'_>) -> Result<(), ProgramError> {
    ...
    pub fn scan<'a>(
        ...
        scan_with(host::storage_scan_shared, prefix, cursor, limits, output)
    }
    /// Drops the complete shared namespace.
    ///
    /// # Errors
    ///
    /// Refuses missing authority or meter.
