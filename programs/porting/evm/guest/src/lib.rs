...
#[cfg(target_arch = "wasm32")]
pub fn msg_sender() -> Result<[u8; 20], ProgramError> {
    match Context::immediate_caller()? {
        Some(caller) => Ok(address(caller.bytes())),
        None => Ok(address(Context::invoking_principal()?.bytes())),
    }
}
/// Returns the executing program as `address(this)`.
#[cfg(target_arch = "wasm32")]
pub fn address_this() -> Result<[u8; 20], ProgramError> {
    Ok(address(Context::executing_program()?.bytes()))
}
/// Returns authenticated batch height as `block.number`.
#[cfg(target_arch = "wasm32")]
pub fn block_number() -> Result<u64, ProgramError> {
    Context::batch_height()
}
/// Calls the ABI v2 Keccak-256 primitive.
#[cfg(target_arch = "wasm32")]
pub fn keccak256(input: HashInput<'_>) -> Result<[u8; 32], ProgramError> {
    crypto::hash(HashAlgorithm::Keccak256, input)
}
...
pub fn ecrecover(
    ...
    let digest = keccak256(HashInput::new(&key[1..])?)?;
    Ok(address(digest))
}
