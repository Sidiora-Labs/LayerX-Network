...
#[cfg(target_arch = "wasm32")]
impl AnchorContext {
    /// Reads the active Anchor context.
    pub fn current() -> Result<Self, ProgramError> {
        Ok(Self {
            program_id: Context::executing_program()?,
            signer: Context::invoking_principal()?,
            slot: Context::batch_height()?,
        })
    }
}
/// Calls the SHA-256 syscall equivalent.
#[cfg(target_arch = "wasm32")]
pub fn sha256(input: HashInput<'_>) -> Result<[u8; 32], ProgramError> {
    crypto::hash(HashAlgorithm::Sha256, input)
}
