...
#[cfg(target_arch = "wasm32")]
pub fn current() -> Result<(Env, MessageInfo), ProgramError> {
    Ok((
        Env {
            contract: Context::executing_program()?,
            block_height: Context::batch_height()?,
        },
        MessageInfo {
            sender: Context::invoking_principal()?,
        },
    ))
}
/// Calls the ABI v2 BLAKE3 primitive.
#[cfg(target_arch = "wasm32")]
pub fn blake3(input: HashInput<'_>) -> Result<[u8; 32], ProgramError> {
    crypto::hash(HashAlgorithm::Blake3, input)
}
