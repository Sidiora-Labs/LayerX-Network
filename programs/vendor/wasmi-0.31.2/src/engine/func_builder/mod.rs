...
impl<'parser> FuncBuilder<'parser> {
    ...
    pub fn translate_locals(
        ...
        self.translator.register_locals(amount, value_type)?;
        Ok(())
    }
    ...
    {
        let operand_types = (0..self.validator.operand_stack_height() as usize)
            .rev()
            .map(|depth| {
                self.validator
                    .get_operand_type(depth)
                    .flatten()
                    .and_then(crate::execution_trace::ExecutionValueType::from_validator)
                    .ok_or_else(|| {
                        TranslationError::unsupported_value_type(wasmparser::ValType::V128)
                    })
            })
    ...
    pub fn set_last_operator_bytes(&mut self, canonical_instruction: &[u8]) {
        self.translator.finish_last_operator(canonical_instruction);
    }
}
