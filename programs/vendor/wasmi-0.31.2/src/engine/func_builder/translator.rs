...
pub struct FuncTranslator<'parser> {
    ...
    unsupported_local_type: Option<ValueType>,
    /// The reusable data structures of the [`FuncTranslator`].
    alloc: FuncTranslatorAllocations,
    last_observe: Option<Instr>,
}

impl<'parser> FuncTranslator<'parser> {
    /// Emits the protocol observation boundary for one validated source operator.
    ...
    pub fn finish_operator(&mut self, instr: Instr, canonical_instruction: &[u8]) {
        self.alloc
            .inst_builder
            .finish_observe(instr, canonical_instruction);
    }

    pub fn finish_last_operator(&mut self, canonical_instruction: &[u8]) {
        let instr = self
            .last_observe
            .take()
            .expect("validated operator must have observation metadata");
        self.finish_operator(instr, canonical_instruction);
    }
    ...
    fn init_func_params(&mut self) {
        for param_type in self.func_type().params() {
            self.locals.register_locals(1);
            let value_type = match param_type {
                ValueType::I32 => crate::execution_trace::ExecutionValueType::I32,
    ...
    fn bump_fuel_consumption(&mut self, delta: u64) -> Result<(), TranslationError> {
        if let Some(observe) = self.last_observe {
            self.alloc.inst_builder.bump_observe_fuel(observe, delta)?;
        }
