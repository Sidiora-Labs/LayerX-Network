...
pub enum WasmOutcome {
    ...
    /// Execution stopped before an observed instruction so the host can meter it.
    Observe(crate::execution_trace::ObservationCharge),
}
...
impl<'ctx, 'engine> Executor<'ctx, 'engine> {
    ...
    pub fn new(
        ...
        let ip = frame.ip();
        let current_func = frame.function();
    ...
    fn capture_execution_snapshot(
        ...
        let instance = self.ctx.resolve_instance(self.cache.instance());
        let mut linear_memory = alloc::vec::Vec::new();
    ...
    fn call_func(
        ...
        match self.ctx.resolve_func(func) {
            FuncEntity::Wasm(wasm_func) => {
                ...
                self.current_func = wasm_func.func_body();
    ...
    fn ret(&mut self, drop_keep: DropKeep) -> ReturnOutcome {
        ...
        match self.call_stack.pop() {
            Some(caller) => {
                ...
                self.current_func = caller.function();
                self.value_base = caller.value_base();
                ReturnOutcome::Wasm
            }
