...
impl Deserialize for Instruction {
    ...
    fn deserialize<R: io::Read>(reader: &mut R) -> Result<Self, Self::Error> {
        ...
        Ok(match val {
            ...
            CURRENTMEMORY => CurrentMemory(VarUint32::deserialize(reader)?.into()),
            GROWMEMORY => GrowMemory(VarUint32::deserialize(reader)?.into()),
            ...
            #[cfg(feature = "reference_types")]
            REF_NULL => RefNull(ValueType::deserialize(reader)?),
            #[cfg(feature = "reference_types")]
...
fn deserialize_bulk<R: io::Read>(reader: &mut R) -> Result<Instruction, Error> {
    ...
    Ok(Instruction::Bulk(match val {
        ...
        MEMORY_FILL => MemoryFill(VarUint32::deserialize(reader)?.into()),
...
impl Serialize for Instruction {
    ...
    fn serialize<W: io::Write>(self, writer: &mut W) -> Result<(), Self::Error> {
        ...
        match self {
            ...
            #[cfg(feature = "reference_types")]
            TableGet(index) => op!(writer, TABLE_GET, VarUint32::from(index).serialize(writer)?),
            #[cfg(feature = "reference_types")]
            TableSet(index) => op!(writer, TABLE_SET, VarUint32::from(index).serialize(writer)?),
            ...
            CurrentMemory(memory) => op!(writer, CURRENTMEMORY, {
                VarUint32::from(memory).serialize(writer)?;
            }),
            GrowMemory(memory) => op!(writer, GROWMEMORY, {
                VarUint32::from(memory).serialize(writer)?;
            }),
            ...
            #[cfg(feature = "reference_types")]
            RefNull(value_type) => op!(writer, REF_NULL, value_type.serialize(writer)?),
            #[cfg(feature = "reference_types")]
            RefIsNull => op!(writer, REF_IS_NULL),
            #[cfg(feature = "reference_types")]
            RefFunc(index) => op!(writer, REF_FUNC, VarUint32::from(index).serialize(writer)?),
...
impl Serialize for BulkInstruction {
    ...
    fn serialize<W: io::Write>(self, writer: &mut W) -> Result<(), Self::Error> {
        ...
        match self {
            MemoryInit(seg, memory) => bulk!(writer, MEMORY_INIT, {
                VarUint32::from(seg).serialize(writer)?;
                VarUint32::from(memory).serialize(writer)?;
            }),
            MemoryDrop(seg) => bulk!(writer, MEMORY_DROP, VarUint32::from(seg).serialize(writer)?),
            MemoryFill(memory) => bulk!(
                writer,
                MEMORY_FILL,
                VarUint32::from(memory).serialize(writer)?
            ),
            MemoryCopy(dst, src) => bulk!(writer, MEMORY_COPY, {
                VarUint32::from(dst).serialize(writer)?;
                VarUint32::from(src).serialize(writer)?;
            }),
            TableInit(seg, table) => bulk!(writer, TABLE_INIT, {
                VarUint32::from(seg).serialize(writer)?;
                VarUint32::from(table).serialize(writer)?;
            }),
            TableDrop(seg) => bulk!(writer, TABLE_DROP, VarUint32::from(seg).serialize(writer)?),
            TableCopy(dst, src) => bulk!(writer, TABLE_COPY, {
                VarUint32::from(dst).serialize(writer)?;
                VarUint32::from(src).serialize(writer)?;
            }),
            #[cfg(feature = "reference_types")]
            TableGrow(table) => bulk!(
                writer,
                TABLE_GROW,
                VarUint32::from(table).serialize(writer)?
            ),
            #[cfg(feature = "reference_types")]
            TableSize(table) => bulk!(
                writer,
                TABLE_SIZE,
                VarUint32::from(table).serialize(writer)?
            ),
            #[cfg(feature = "reference_types")]
            TableFill(table) => bulk!(
                writer,
                TABLE_FILL,
                VarUint32::from(table).serialize(writer)?
            ),
...
impl fmt::Display for Instruction {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            #[cfg(feature = "reference_types")]
            RefNull(value_type) => fmt_op!(f, "ref.null", value_type),
            #[cfg(feature = "reference_types")]
            RefIsNull => fmt_op!(f, "ref.is_null"),
            #[cfg(feature = "reference_types")]
            RefFunc(index) => fmt_op!(f, "ref.func", index),
...
impl fmt::Display for BulkInstruction {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            MemoryInit(_, _) => write!(f, "memory.init"),
            MemoryDrop(_) => write!(f, "memory.drop"),
            MemoryFill(_) => write!(f, "memory.fill"),
            MemoryCopy(_, _) => write!(f, "memory.copy"),
            TableInit(_, _) => write!(f, "table.init"),
            TableDrop(_) => write!(f, "table.drop"),
            TableCopy(_, _) => write!(f, "table.copy"),
            #[cfg(feature = "reference_types")]
            TableGrow(index) => write!(f, "table.grow {}", index),
            #[cfg(feature = "reference_types")]
            TableSize(index) => write!(f, "table.size {}", index),
            #[cfg(feature = "reference_types")]
            TableFill(index) => write!(f, "table.fill {}", index),
        }
    }
}
