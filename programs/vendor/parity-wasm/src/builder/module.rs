...
impl From<ModuleScaffold> for elements::Module {
    fn from(module: ModuleScaffold) -> Self {
        ...
        if let Some(data_count) = module.data_count {
            sections.push(elements::Section::DataCount(data_count));
        }
    ...
    F: Invoke<elements::Module>,
{
    ...
    pub fn push_function(&mut self, func: code::FunctionDefinition) -> CodeLocation {
        ...
        self.module
            .functions
            .entries_mut()
            .push(elements::Func::new(type_ref));
    ...
    pub fn push_table(&mut self, mut table: table::TableDefinition) -> u32 {
        ...
        for entry in table.elements.drain(..) {
            self.module
                .element
                .entries_mut()
                .push(elements::ElementSegment::new(
                    table_index,
                    Some(entry.offset),
                    entry.values,
                ))
        ...
        match signature {
            code::Signature::Inline(func_type) => {
                ...
                self.module
                    .types
                    .types_mut()
                    .push(elements::Type::Function(func_type));
