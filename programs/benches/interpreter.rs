...
fn execute(
    ...
    input: &[u8],
) -> (Storage, CandidateAuthorizedExecutionRecord) {
    let program = ProgramId::new([0x35; 32]).unwrap_or_else(|error| panic!("program id: {error}"));
    let principal =
        PrincipalId::new([0x53; 32]).unwrap_or_else(|error| panic!("principal id: {error}"));
...
fn assert_equivalent(
    ...
    assert_eq!(
        interpreted.0, compiled.0,
        "{workload}: committed state differs"
    );
    ...
    assert_eq!(
        left.root_program(),
        right.root_program(),
        "{workload}: receipt program differs"
    );
    assert_eq!(
        left.abi_revision(),
        right.abi_revision(),
        "{workload}: receipt ABI differs"
    );
    assert_eq!(
        left.runtime_version(),
        right.runtime_version(),
        "{workload}: runtime version differs"
    );
    assert_eq!(
        left.fee_schedule_version(),
        right.fee_schedule_version(),
        "{workload}: fee schedule differs"
    );
    assert_eq!(
        left.metering_schedule_version(),
        right.metering_schedule_version(),
        "{workload}: metering schedule differs"
    );
    assert_eq!(
        left.graph_evidence(),
        right.graph_evidence(),
        "{workload}: call graph differs"
    );
    assert_eq!(
        left.outcome(),
        right.outcome(),
        "{workload}: receipt outcome differs modulo cost"
    );
    match (interpreted.1.outcome(), compiled.1.outcome()) {
        (
            CandidateActivityOutcome::Success { effects: left, .. },
            CandidateActivityOutcome::Success { effects: right, .. },
        ) => {
...
fn programs_interpreter_bench() {
    ...
    assert!(
        ...
        "sample count must be an odd integer of at least three"
    );
    ...
    let interpreter = engine
        .validate_v2(&interpreter)
        .unwrap_or_else(|error| panic!("validate interpreter: {error}"));
    let compiled = engine
        .validate_v2(&compiled)
        .unwrap_or_else(|error| panic!("validate compiled equivalent: {error}"));
    ...
    for workload in WORKLOADS {
        let script = vectors
            .get(workload.vector)
            .unwrap_or_else(|| panic!("missing vector {}", workload.vector));
        ...
        print_usage(
            "interpreted",
            workload.name,
            interpreted_record.1.execution().usage(),
        );
        print_usage(
            "compiled",
            workload.name,
            compiled_record.1.execution().usage(),
        );
        interpreted_fee_units = interpreted_fee_units
            .checked_add(interpreted_record.1.execution().usage().fee_units)
            .unwrap_or_else(|| panic!("interpreted fee overflow"));
        compiled_fee_units = compiled_fee_units
            .checked_add(compiled_record.1.execution().usage().fee_units)
            .unwrap_or_else(|| panic!("compiled fee overflow"));
        let interpreted_ns = timed(samples, || {
            let _ = execute(&interpreter, script);
        });
        let compiled_ns = timed(samples, || {
            let _ = execute(&compiled, workload.compiled_input);
        });
        interpreted_total = interpreted_total
            .checked_add(interpreted_ns)
            .unwrap_or_else(|| panic!("interpreted duration overflow"));
        compiled_total = compiled_total
            .checked_add(compiled_ns)
            .unwrap_or_else(|| panic!("compiled duration overflow"));
        println!(
            "timing workload={} interpreted_median_ns={} compiled_median_ns={}",
            workload.name, interpreted_ns, compiled_ns
        );
    }
    assert_ne!(
        compiled_total, 0,
        "compiled benchmark clock resolution is insufficient"
    );
    assert_ne!(
        compiled_fee_units, 0,
        "compiled workload consumed zero protocol fee units"
    );
    let observed_time_bps = interpreted_total
        .checked_mul(BPS)
        .unwrap_or_else(|| panic!("time ratio overflow"))
        / compiled_total;
    let observed_fee_bps = interpreted_fee_units
        .checked_mul(BPS)
        .unwrap_or_else(|| panic!("fee ratio overflow"))
        / compiled_fee_units;
    let time_gate_bps = PUBLISHED_TIME_MULTIPLIER_BPS
        .checked_mul(BPS + TOLERANCE_BPS)
        .unwrap_or_else(|| panic!("time gate overflow"))
        / BPS;
    let fee_gate_bps = PUBLISHED_FEE_MULTIPLIER_BPS
        .checked_mul(BPS + TOLERANCE_BPS)
        .unwrap_or_else(|| panic!("fee gate overflow"))
        / BPS;
