use layerx_programs::{interface_state_value, lxt721::reference_interface};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if arguments.len() != 5 {
        return Err(
            "usage: lxt721_interface WASM PROGRAM_HEX INTERFACE_OUT STATE_VALUE_OUT".into(),
        );
    }
    ...
    let interface = reference_interface(&fs::read(&arguments[1])?)?;
