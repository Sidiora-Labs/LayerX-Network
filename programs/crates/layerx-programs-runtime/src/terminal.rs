...
use crate::{
    BudgetMeterRefusal, BudgetResourceKind, MeteredUsage, ProgramFailure, TransferLawError,
    DEFAULT_MAX_CALL_GRAPH_EDGES, MAX_CALL_RESPONSE_BYTES, MAX_TRACE_COMMITMENTS,
};
...
const MAX_GRAPH_EVIDENCE_BYTES: usize = b"LayerX/programs/call-graph/v1\0".len()
    ...
    + 16
    + 8
    + DEFAULT_MAX_CALL_GRAPH_EDGES as usize * 68;
...
pub enum ExecutionTerminal {
    ...
    /// Frozen ABI-v2 receipt encoding v4; retains its historical public variant name.
    CandidateV4 {
        runtime_version: u16,
...
pub fn decode_terminal_payload(
    ...
    let detail = if inner.starts_with(EXECUTION_V2) || inner.starts_with(EXECUTION_V3) && abi == 1 {
        ...
        let expected = match &execution {
            ...
            ExecutionTerminal::CandidateV4 {
                outcome: CandidateTerminalOutcome::Failure(_),
                ..
            } => 2,
            ExecutionTerminal::CandidateV4 {
                outcome: CandidateTerminalOutcome::Resource(_),
                ..
            } => 3,
        ...
        if kind != 2 || inner.len() != SETTLEMENT.len() + 1 {
            return Err(TerminalDecodeError::Malformed);
        }
        TerminalDetail::Failure(FailureTerminal::Settlement(transfer_error(
            inner[SETTLEMENT.len()],
        )?))
    } else if inner.starts_with(CALLBACK) {
        if kind != 2 || inner.len() != CALLBACK.len() + 5 {
            return Err(TerminalDecodeError::Malformed);
        }
        TerminalDetail::Failure(FailureTerminal::Callback {
            stage: inner[CALLBACK.len()],
            status: i32::from_be_bytes(
                inner[CALLBACK.len() + 1..]
                    .try_into()
...
fn unwrap(encoded: &[u8]) -> Result<(&[u8], Vec<TerminalAttachment>), TerminalDecodeError> {
    ...
    if inner.starts_with(AUTHORITY) {
        ...
        let execution = cursor.sized_u32()?;
        let authorization = cursor.sized_u32()?.to_vec();
        let transfer_root = cursor.array()?;
        cursor.end()?;
        attachments.push(TerminalAttachment::TransferAuthority {
            authorization,
            transfer_root,
        });
        inner = execution;
    ...
    if inner.starts_with(OCCUPANCY) {
        ...
        let execution = cursor.sized_u32()?;
        let occupancy = cursor.sized_u32()?.to_vec();
        cursor.end()?;
        attachments.push(TerminalAttachment::Occupancy(occupancy));
        inner = execution;
    }
    if inner.starts_with(AUTHORITY) || inner.starts_with(OCCUPANCY) {
        return Err(TerminalDecodeError::Malformed);
    }
...
fn decode_legacy(
    ...
    expected_abi: u16,
    traced: bool,
) -> Result<ExecutionTerminal, TerminalDecodeError> {
    ...
    if abi_version != expected_abi || runtime_version == 0 || metering_schedule_version == 0 {
        return Err(TerminalDecodeError::MismatchedAbi);
    }
    ...
    c.end()?;
    Ok(ExecutionTerminal::Legacy {
        encoding_version: if traced { 3 } else { 2 },
        runtime_version,
        abi_version,
...
fn decode_failure(encoded: &[u8]) -> Result<FailureTerminal, TerminalDecodeError> {
    ...
    let tag = c.byte()?;
    let payload = c.sized_u32()?;
    c.end()?;
    Ok(match tag {
        1 => FailureTerminal::Program(
            ProgramFailure::canonical_decode(payload)
                .map_err(|_| TerminalDecodeError::Malformed)?,
        ),
        2 => {
            let (tag, fields) = decode_composition(payload)?;
            FailureTerminal::Composition { tag, fields }
        }
        3 => {
            let (tag, fields) = decode_entrypoint(payload)?;
            FailureTerminal::Entrypoint { tag, fields }
        }
        4 => {
            let (tag, fields) = decode_abi(payload)?;
            FailureTerminal::Abi { tag, fields }
        }
        _ => return Err(TerminalDecodeError::Malformed),
    })
}
...
fn decode_composition(payload: &[u8]) -> Result<(u8, FailureFields), TerminalDecodeError> {
    ...
    let fields = match tag {
        ...
        2 => {
            let expected = c.byte()?;
            let actual = c.byte()?;
            if !matches!(expected, 1 | 2) || !matches!(actual, 1 | 2) {
                return Err(TerminalDecodeError::Malformed);
            }
            FailureFields::Revisions { expected, actual }
        }
        23 => FailureFields::MeteringPlans {
            expected: c.array()?,
            actual: c.array()?,
        },
        ...
        8 => FailureFields::ProgramBounds {
            program: c.array()?,
            limit: c.u32()?,
            attempted: c.u32()?,
        },
        12 => FailureFields::Code(c.i32()?),
        13 => FailureFields::Sizes {
            first: c.u64()?,
            second: c.u64()?,
        },
        14 => FailureFields::ProgramCode {
            program: c.array()?,
            code: c.i32()?,
        },
        15 => FailureFields::ProgramFailure(
            ProgramFailure::canonical_decode(c.rest())
                .map_err(|_| TerminalDecodeError::Malformed)?,
        ),
...
fn decode_abi(payload: &[u8]) -> Result<(u8, FailureFields), TerminalDecodeError> {
    ...
    let fields = match tag {
        ...
        11 => {
            let storage = c.byte()?;
            if !matches!(storage, 1..=11) {
                return Err(TerminalDecodeError::Malformed);
            }
            FailureFields::Abi(AbiFailure::Storage(storage))
        }
        12 => FailureFields::Meter(decode_meter_failure(&mut c)?),
        _ => return Err(TerminalDecodeError::Malformed),
    };
    c.end()?;
    Ok((tag, fields))
}
fn decode_abi_fields(c: &mut Cursor<'_>) -> Result<AbiFailure, TerminalDecodeError> {
    let tag = c.byte()?;
    Ok(match tag {
        1..=10 | 13..=15 => AbiFailure::Simple(tag),
        11 => {
            let storage = c.byte()?;
            if !matches!(storage, 1..=11) {
                return Err(TerminalDecodeError::Malformed);
            }
            AbiFailure::Storage(storage)
        }
        12 => AbiFailure::Meter(decode_meter_failure(c)?),
        _ => return Err(TerminalDecodeError::Malformed),
    })
}
fn decode_meter_failure(c: &mut Cursor<'_>) -> Result<MeterFailure, TerminalDecodeError> {
    let tag = c.byte()?;
    Ok(match tag {
        1 => {
            let resource = c.byte()?;
            if !matches!(resource, 1..=7) {
                return Err(TerminalDecodeError::Malformed);
            }
            let limit = c.u64()?;
            let attempted = c.u64()?;
        ...
        2 => {
            ...
            MeterFailure::CounterOverflow { resource }
        }
        3 => MeterFailure::FeeOverflow,
        _ => return Err(TerminalDecodeError::Malformed),
    })
}
fn decode_fault(c: &mut Cursor<'_>) -> Result<ExecutionFaultFailure, TerminalDecodeError> {
    let tag = c.byte()?;
    Ok(match tag {
        1 | 2 | 16 => {
            let bytes = c.sized_u32()?;
            let name = core::str::from_utf8(bytes)
                .map_err(|_| TerminalDecodeError::Malformed)?
                .to_owned();
            ExecutionFaultFailure::Named { tag, name }
        }
        3..=13 | 15 => ExecutionFaultFailure::Simple(tag),
        14 => ExecutionFaultFailure::Resource(decode_meter_failure(c)?),
        _ => return Err(TerminalDecodeError::Malformed),
    })
}
fn decode_response(c: &mut Cursor<'_>) -> Result<ResponseFailure, TerminalDecodeError> {
    let tag = c.byte()?;
    Ok(match tag {
        1 | 2 => ResponseFailure::Sizes {
            tag,
            bytes: c.u64()?,
            bound: c.u64()?,
        },
        3 | 4 => ResponseFailure::Simple(tag),
        5 => ResponseFailure::CodeMismatch {
            published: c.i32()?,
            returned: c.i32()?,
        },
        6 => ResponseFailure::Meter(decode_meter_failure(c)?),
        _ => return Err(TerminalDecodeError::Malformed),
    })
}
...
fn decode_resource(encoded: &[u8]) -> Result<BudgetMeterRefusal, TerminalDecodeError> {
    ...
    let result = match tag {
        ...
        2 => BudgetMeterRefusal::CounterOverflow { resource },
        _ => return Err(TerminalDecodeError::Malformed),
    };
    c.end()?;
    Ok(result)
...
fn decode_meter(
    c: &mut Cursor<'_>,
    usage: MeteredUsage,
) -> Result<BudgetMeterRefusal, TerminalDecodeError> {
    let tag = c.byte()?;
    let resource = candidate_resource(c.byte()?)?;
    match tag {
        0 => {
            let limit = c.u64()?;
            let attempted = c.u64()?;
            if attempted <= limit || usage_for(usage, resource) > limit {
                return Err(TerminalDecodeError::Malformed);
            }
        ...
        _ => Err(TerminalDecodeError::Malformed),
    }
}
fn resource(tag: u8) -> Result<BudgetResourceKind, TerminalDecodeError> {
    Ok(match tag {
        1 => BudgetResourceKind::Cpu,
        2 => BudgetResourceKind::Memory,
        3 => BudgetResourceKind::StorageRead,
        4 => BudgetResourceKind::StorageWrite,
        5 => BudgetResourceKind::Output,
        6 => BudgetResourceKind::OutputBytes,
        7 => BudgetResourceKind::Table,
        _ => return Err(TerminalDecodeError::Malformed),
    })
}
fn candidate_resource(tag: u8) -> Result<BudgetResourceKind, TerminalDecodeError> {
    Ok(match tag {
...
fn usage_for(usage: MeteredUsage, resource: BudgetResourceKind) -> u64 {
    match resource {
        BudgetResourceKind::Cpu => usage.cpu_fuel,
        BudgetResourceKind::Memory => usage.memory_bytes,
        BudgetResourceKind::StorageRead => usage.storage_read_bytes,
        BudgetResourceKind::StorageWrite => usage.storage_write_bytes,
        BudgetResourceKind::Output => u64::from(usage.output_values),
        BudgetResourceKind::OutputBytes => usage.output_bytes,
        BudgetResourceKind::Table => 0,
    }
}
fn transfer_error(tag: u8) -> Result<TransferLawError, TerminalDecodeError> {
    Ok(match tag {
        1 => TransferLawError::UnverifiedAuthority,
        11 => TransferLawError::InvalidProgramAuthority,
        12 => TransferLawError::InvalidProgramFunding,
        2 => TransferLawError::InvalidTransfer,
        3 => TransferLawError::InvalidTransferSet,
        4 => TransferLawError::AmountOverflow,
        5 => TransferLawError::InvariantViolation,
        6 => TransferLawError::CapabilityEscalation,
        7 => TransferLawError::KernelRefused,
        8 => TransferLawError::ReceiptInvalid,
        9 => TransferLawError::ReceiptMismatch,
        10 => TransferLawError::StaleStorage,
        _ => return Err(TerminalDecodeError::Malformed),
    })
}
...
impl<'a> Cursor<'a> {
    ...
    fn take(&mut self, n: usize) -> Result<&'a [u8], TerminalDecodeError> {
        let end = self
            .offset
            .checked_add(n)
            .ok_or(TerminalDecodeError::Malformed)?;
        let out = self
            .bytes
            .get(self.offset..end)
            .ok_or(TerminalDecodeError::Malformed)?;
    ...
    fn array<const N: usize>(&mut self) -> Result<[u8; N], TerminalDecodeError> {
        self.take(N)?
            .try_into()
            .map_err(|_| TerminalDecodeError::Malformed)
    }
    fn byte(&mut self) -> Result<u8, TerminalDecodeError> {
        Ok(self.take(1)?[0])
    }
    fn u16(&mut self) -> Result<u16, TerminalDecodeError> {
        Ok(u16::from_be_bytes(self.array()?))
    }
    fn u32(&mut self) -> Result<u32, TerminalDecodeError> {
        Ok(u32::from_be_bytes(self.array()?))
    }
    fn u64(&mut self) -> Result<u64, TerminalDecodeError> {
        Ok(u64::from_be_bytes(self.array()?))
    }
    fn i32(&mut self) -> Result<i32, TerminalDecodeError> {
        Ok(i32::from_be_bytes(self.array()?))
    }
    fn i64(&mut self) -> Result<i64, TerminalDecodeError> {
        Ok(i64::from_be_bytes(self.array()?))
    }
    fn sized_u32(&mut self) -> Result<&'a [u8], TerminalDecodeError> {
        let n = usize::try_from(self.u32()?).map_err(|_| TerminalDecodeError::Malformed)?;
        self.take(n)
    }
    fn sized_u64(&mut self) -> Result<&'a [u8], TerminalDecodeError> {
        let n = usize::try_from(self.u64()?).map_err(|_| TerminalDecodeError::Malformed)?;
        self.take(n)
    }
    fn end(&self) -> Result<(), TerminalDecodeError> {
        if self.offset == self.bytes.len() {
            Ok(())
        } else {
            Err(TerminalDecodeError::Malformed)
        }
    }
...
mod source_vectors {
    ...
    fn current_candidate_v4_success_decodes_exactly() {
        ...
        bytes.extend_from_slice(&0_u64.to_be_bytes());
        let Ok(decoded) = decode_terminal_payload(1, 2, &bytes) else {
            panic!("producer vector rejected")
        };
        assert!(
    ...
    fn traced_abi_one_execution_v3_is_not_candidate_evidence() {
        ...
        bytes.push(1);
        bytes.extend_from_slice(&0_u64.to_be_bytes());
        let Ok(decoded) = decode_terminal_payload(1, 1, &bytes) else {
            panic!("traced ABI1 producer vector rejected")
        };
        assert!(matches!(
            decoded.detail,
