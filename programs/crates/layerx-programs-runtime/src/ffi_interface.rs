...
enum ValueType {
    ...
    I128,
    Bytes(u32),
    FixedArray(u32, Box<Self>),
    VariableArray(u32, Box<Self>),
    Option(Box<Self>),
    Union(Vec<Variant>),
}
#[derive(Clone, Eq, PartialEq)]
...
fn bytes(token: u64, section: u16, length: usize, maximum: usize) -> Result<Vec<u8>, i32> {
    if token == 0 || length == 0 || length > maximum {
...
fn schema(input: &[u8], cursor: &mut usize, depth: usize) -> Result<Schema, i32> {
    ...
    let convention =
        EncodingConvention::from_tag(take::<1>(input, cursor)?[0]).map_err(|_| NON_CANONICAL)?;
    let value = match convention {
        EncodingConvention::LayerX => Some(value_type(input, cursor, depth)?),
...
fn value_type(input: &[u8], cursor: &mut usize, depth: usize) -> Result<ValueType, i32> {
    ...
    Ok(
        match TypeTag::from_byte(take::<1>(input, cursor)?[0]).map_err(|_| NON_CANONICAL)? {
            ...
            TypeTag::Bytes => {
                let n = u32::from_be_bytes(take::<4>(input, cursor)?);
                if n == 0 {
                    return Err(NON_CANONICAL);
                }
                ValueType::Bytes(n)
            ...
            TypeTag::FixedArray => {
                ...
                ValueType::FixedArray(n, Box::new(value_type(input, cursor, depth + 1)?))
            }
            TypeTag::VariableArray => {
                let n = u32::from_be_bytes(take::<4>(input, cursor)?);
                if n == 0 {
                    return Err(NON_CANONICAL);
                }
                ValueType::VariableArray(n, Box::new(value_type(input, cursor, depth + 1)?))
            }
            TypeTag::Option => ValueType::Option(Box::new(value_type(input, cursor, depth + 1)?)),
            TypeTag::Union => {
                let n = count(input, cursor)?;
                if n == 0 {
                    return Err(NON_CANONICAL);
                }
                ...
                for _ in 0..n {
                    v.push(Variant {
                        tag: u32::from_be_bytes(take::<4>(input, cursor)?),
                        value: value_type(input, cursor, depth + 1)?,
                    });
                }
...
fn decode(input: &[u8]) -> Result<Interface, i32> {
    ...
    for _ in 0..n {
        let name = text(input, &mut c)?;
        let discriminator = take::<4>(input, &mut c)?;
        let calldata = schema(input, &mut c, 0)?;
        let response = schema(input, &mut c, 0)?;
        ...
        let tn = count(input, &mut c)?;
        let mut topics = Vec::with_capacity(tn);
        for _ in 0..tn {
            topics.push(take::<32>(input, &mut c)?);
        }
        if !topics.windows(2).all(|p| p[0] < p[1]) {
            return Err(NON_CANONICAL);
        }
        let en = count(input, &mut c)?;
        let mut failures = Vec::with_capacity(en);
        for _ in 0..en {
            failures.push(Failure {
                code: u32::from_be_bytes(take::<4>(input, &mut c)?),
                name: text(input, &mut c)?,
                schema: schema(input, &mut c, 0)?,
            });
        }
...
fn capability(input: &[u8], cursor: &mut usize) -> Result<Vec<u8>, i32> {
    ...
    match take::<1>(input, cursor)?[0] {
        ...
        7 => {
            ...
            let n = usize::from(u16::from_be_bytes(take::<2>(input, cursor)?));
            if n == 0 || n > crate::MAX_PROGRAM_ACCOUNT_SEED_BYTES {
                return Err(NON_CANONICAL);
            }
            let end = cursor.checked_add(n).ok_or(NON_CANONICAL)?;
            ...
            *cursor = end;
            let source = take::<32>(input, cursor)?;
            let asset = take::<32>(input, cursor)?;
            let to = take::<32>(input, cursor)?;
            if source == [0; 32]
                || asset == [0; 32]
        ...
        9 => {
            let account = take::<32>(input, cursor)?;
            let asset = take::<32>(input, cursor)?;
    ...
    Ok(input[start..*cursor].to_vec())
}
fn accepts(new: &Schema, old: &Schema) -> bool {
    new.convention == old.convention
        && match (&new.value, &old.value) {
            (None, None) => true,
            (Some(a), Some(b)) => accepts_value(a, b),
            _ => false,
        }
}
fn accepts_value(new: &ValueType, old: &ValueType) -> bool {
    match (new, old) {
        ...
        (ValueType::FixedArray(an, a), ValueType::FixedArray(bn, b)) => {
            an == bn && accepts_value(a, b)
        }
        (ValueType::VariableArray(an, a), ValueType::VariableArray(bn, b)) => {
            an >= bn && accepts_value(a, b)
        }
        (ValueType::Option(a), ValueType::Option(b)) => accepts_value(a, b),
        (ValueType::Union(a), ValueType::Union(b)) => b.iter().all(|old| {
            a.iter()
                .any(|new| new.tag == old.tag && accepts_value(&new.value, &old.value))
        }),
        _ => false,
    }
}
            ...
            new.entries
                ...
                .is_some_and(|n| {
                    n.discriminator == o.discriminator
                        && accepts(&n.calldata, &o.calldata)
                        && accepts(&o.response, &n.response)
                        && n.capabilities.iter().all(|x| o.capabilities.contains(x))
                        && o.topics.iter().all(|x| n.topics.contains(x))
...
pub extern "C" fn layerx_programs_interface_validate(
    ...
    let wasm = match bytes(token, 0, wasm_length as usize, MAX_MODULE_BYTES) {
        Ok(x) => x,
        Err(e) => return e,
    };
    let encoded = match bytes(token, 2, interface_length as usize, MAX_INTERFACE_BYTES) {
        Ok(x) => x,
        Err(e) => return e,
    };
    let interface = match decode(&encoded) {
        Ok(x) => x,
        Err(e) => return e,
    };
    let mut hash = [0u8; 32];
    for (c, w) in hash.chunks_exact_mut(8).zip([h0, h1, h2, h3]) {
        c.copy_from_slice(&w.to_be_bytes());
    }
    if interface.hash != hash
        || interface.abi != abi
        || <[u8; 32]>::from(Sha256::digest(&wasm)) != hash
    {
    ...
    if interface.entries.iter().any(|e| {
        !module.supports_interface_entrypoint(&e.name)
            || !module.interface_capability_mask_matches(&e.name, capability_mask(&e.capabilities))
    }) {
        return NON_CANONICAL;
    }
    if prior_length > 0 {
        let prior = match bytes(token, 3, prior_length as usize, MAX_INTERFACE_BYTES)
            .and_then(|x| decode(&x))
        {
            Ok(x) => x,
            Err(e) => return e,
