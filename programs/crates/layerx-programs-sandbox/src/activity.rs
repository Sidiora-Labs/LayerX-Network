...
use crate::{
    AuthenticatedUsageReceipt, Lease, LeaseId, LeaseState, UsageReceipt,
...
pub struct SandboxProtocolContext {
    protocol_version: u16,
    active_programs_abi: u16,
}

impl SandboxProtocolContext {
    /// # Errors
    ///
    /// Returns a protocol mismatch when the protocol or active Programs ABI is unsupported.
    pub fn new(
        protocol_version: u16,
        active_programs_abi: u16,
...
impl SandboxSubmission {
    #[must_use]
    pub const fn activity_type(&self) -> u32 {
        self.activity_type
    }
    #[must_use]
    ...
    #[must_use]
    pub const fn expected_usage_sequence(&self) -> u64 {
    ...
    fn decode_funding(
        ...
        let transfer_length = usize::try_from(u32::from_be_bytes(
            payload[lease_end..transfer_length_end]
                .try_into()
                .map_err(|_| SandboxActivityRefusal::NonCanonical)?,
        ))
        .map_err(|_| SandboxActivityRefusal::LengthLimit)?;
        if transfer_length == 0
            || transfer_length_end.checked_add(transfer_length) != Some(payload.len())
        {
            return Err(SandboxActivityRefusal::NonCanonical);
        }
        ...
        Ok(Self {
            activity_type: PROGRAMS_SANDBOX_ACTIVITY_TYPE,
            payload: payload.to_vec(),
            lease,
            expected_usage_sequence: 0,
    ...
    /// # Errors
    ///
    /// Returns a refusal for malformed, noncanonical or inconsistent submission fields.
    pub fn decode(
        payload: &[u8],
        protocol: SandboxProtocolContext,
    ) -> Result<Self, SandboxActivityRefusal> {
        ...
        if payload[1] == OP_ACTIVATE {
            ...
            let expected_lease_digest = payload[36..68]
                .try_into()
                .map_err(|_| SandboxActivityRefusal::NonCanonical)?;
            let expected_usage_sequence = u64::from_be_bytes(
                payload[68..76]
                    .try_into()
                    .map_err(|_| SandboxActivityRefusal::NonCanonical)?,
            );
            ...
            return Ok(Self {
                ...
                payload: payload.to_vec(),
                lease,
                expected_usage_sequence,
                expected_lease_digest,
                protocol,
            });
        }
        if payload[1] == OP_FUND {
            return Self::decode_funding(payload, protocol, lease);
        }
        ...
        let expected_lease_digest = payload[44..76]
            .try_into()
            .map_err(|_| SandboxActivityRefusal::NonCanonical)?;
        if payload[76..108] == [0; 32]
            || payload[108..140] == [0; 32]
            || payload[140..172] == [0; 32]
        {
            return Err(SandboxActivityRefusal::NonCanonical);
        }
        let call_length = usize::try_from(u32::from_be_bytes(
            payload[232..236]
                .try_into()
                .map_err(|_| SandboxActivityRefusal::NonCanonical)?,
        ))
        .map_err(|_| SandboxActivityRefusal::LengthLimit)?;
        if expected_usage_sequence == 0
            ...
            || call_length == 0
            || call_length > MAX_CANONICAL_CALL_BYTES
...
/// # Errors
///
/// Returns a refusal when lease state, funding amounts or canonical transfer encoding are invalid.
pub fn canonical_fund(
    protocol: SandboxProtocolContext,
    lease: &Lease,
    canonical_programs_transfer: &[u8],
    ...
    let leg_count = u16::from_be_bytes(
        canonical_programs_transfer[32..34]
            .try_into()
            .map_err(|_| SandboxActivityRefusal::NonCanonical)?,
    );
    if canonical_programs_transfer[..32] != lease.host_program().bytes()
        || leg_count != 1
        ...
        || u128::from_be_bytes(
            canonical_programs_transfer[130..146]
                .try_into()
                .map_err(|_| SandboxActivityRefusal::NonCanonical)?,
        ) != lease.escrow_amount()
    {
    ...
    payload.extend_from_slice(
        &u32::try_from(lease_bytes.len())
            .map_err(|_| SandboxActivityRefusal::LengthLimit)?
            .to_be_bytes(),
    );
    payload.extend_from_slice(&lease_bytes);
    payload.extend_from_slice(
        &u32::try_from(canonical_programs_transfer.len())
            .map_err(|_| SandboxActivityRefusal::LengthLimit)?
            .to_be_bytes(),
    );
...
/// # Errors
///
/// Returns a refusal when the lease cannot be activated or its state cannot be encoded.
pub fn canonical_activate(
    protocol: SandboxProtocolContext,
    lease: &Lease,
) -> Result<SandboxSubmission, SandboxActivityRefusal> {
    if lease.state() != LeaseState::Funded {
        return Err(SandboxActivityRefusal::LeaseNotActive);
    }
    let digest = lease
        .state_digest()
        .map_err(|_| SandboxActivityRefusal::StateDigest)?;
    let next = u64::try_from(lease.history().len())
        .map_err(|_| SandboxActivityRefusal::LengthLimit)?
        .checked_add(1)
        .ok_or(SandboxActivityRefusal::LengthLimit)?;
...
impl SandboxSubmissionReceipt {
    /// # Errors
    ///
    /// Returns a refusal when protocol-head verification or receipt binding fails.
    pub fn verify(
        ...
        Ok(Self {
            ...
            expected_usage_sequence: submission.expected_usage_sequence,
    ...
    /// # Errors
    ///
    /// Returns a refusal when receipt chunks, usage encoding or submission bindings are invalid.
    pub fn authenticated_usage_receipt(
        &self,
        ...
        let protocol = decoded
            .protocol()
            .ok_or(SandboxActivityRefusal::InvalidReceipt)?;
        ...
        for effect in protocol.effects().iter().filter(|effect| {
            ...
            let declared_total = usize::try_from(u32::from_be_bytes(
                body[48..52]
                    .try_into()
                    .map_err(|_| SandboxActivityRefusal::InvalidReceipt)?,
            ))
            ...
            if count == 0
                ...
                || count != declared_total.div_ceil(SANDBOX_USAGE_CHUNK_BYTES)
                || body.len() - SANDBOX_USAGE_FRAME_BYTES
            ...
            if chunks.is_empty() {
                chunks.resize_with(count, || None);
            }
            if chunks[index]
                .replace(body[SANDBOX_USAGE_FRAME_BYTES..].to_vec())
                .is_some()
            {
                return Err(SandboxActivityRefusal::InvalidReceipt);
            }
...
pub trait SandboxActivityPlane {
    ...
    /// # Errors
    ///
    /// Returns the activity-plane error when submission cannot be accepted.
    fn submit_sandbox(
        &mut self,
        submission: SandboxSubmission,
...
/// # Errors
///
/// Returns a refusal when canonical execution construction or activity-plane submission fails.
pub fn execute<P: SandboxActivityPlane>(
    ...
    let submission = canonical_execute(
        protocol,
        lease,
        expected_usage_sequence,
        canonical_programs_call,
    )
    .map_err(SandboxExecuteRefusal::Activity)?;
    let evidence = plane
        .submit_sandbox(submission.clone())
        .map_err(SandboxExecuteRefusal::Submission)?;
...
/// # Errors
///
/// Returns a refusal when canonical funding construction or activity-plane submission fails.
...
/// # Errors
///
/// Returns a refusal when canonical activation construction or activity-plane submission fails.
pub fn activate_lease<P: SandboxActivityPlane>(
    ...
    let submission =
        canonical_activate(protocol, lease).map_err(SandboxExecuteRefusal::Activity)?;
    let evidence = plane
        .submit_sandbox(submission.clone())
        .map_err(SandboxExecuteRefusal::Submission)?;
    SandboxSubmissionReceipt::verify(verifier, &submission, evidence, now_ms)
...
fn verify_submission_activity(
    ...
    let registration = ModuleRegistration::new(ModuleId::Programs, &[sandbox])
        .map_err(|_| SandboxActivityRefusal::InvalidReceipt)?;
    let registry =
        ModuleRegistry::new(&[registration]).map_err(|_| SandboxActivityRefusal::InvalidReceipt)?;
    ...
    if encode_signed(&activity).map_err(|_| SandboxActivityRefusal::InvalidReceipt)?
        ...
        || activity.activity_type() != sandbox
        || activity.payload() != submission.payload
...
/// # Errors
///
/// Returns a refusal for invalid lease state, call encoding, authority, limits or state bindings.
pub fn canonical_execute(
    ...
    if expected_usage_sequence == 0
        || canonical_programs_call.is_empty()
    ...
    let call_length = u32::try_from(canonical_programs_call.len())
        .map_err(|_| SandboxActivityRefusal::LengthLimit)?;
    let expected_lease_digest = lease
        .state_digest()
        .map_err(|_| SandboxActivityRefusal::StateDigest)?;
...
fn validate_call_structure(
    ...
    let entrypoint_length = usize::from(u16::from_be_bytes(
        call[34..36]
            .try_into()
            .map_err(|_| SandboxActivityRefusal::NonCanonical)?,
    ));
    ...
    .map_err(|_| SandboxActivityRefusal::LengthLimit)?;
    let capabilities_length = usize::from(u16::from_be_bytes(
        call[40..42]
            .try_into()
            .map_err(|_| SandboxActivityRefusal::NonCanonical)?,
    ));
    let access_length = usize::try_from(u32::from_be_bytes(
        call[42..46]
            .try_into()
            .map_err(|_| SandboxActivityRefusal::NonCanonical)?,
    ))
    .map_err(|_| SandboxActivityRefusal::LengthLimit)?;
    let response_capacity = usize::try_from(u32::from_be_bytes(
        call[46..50]
            .try_into()
            .map_err(|_| SandboxActivityRefusal::NonCanonical)?,
    ))
    .map_err(|_| SandboxActivityRefusal::LengthLimit)?;
    let capabilities_start = CALL_FIXED_BYTES
        .checked_add(entrypoint_length)
        .and_then(|value| value.checked_add(calldata_length))
        .ok_or(SandboxActivityRefusal::LengthLimit)?;
    let access_start = capabilities_start
    ...
    if entrypoint.is_empty()
        ...
        || capabilities_length > 65_535
        || access_length > 1_048_576
    ...
    decode_call_limits(call)
}

fn decode_call_limits(call: &[u8]) -> Result<[u64; 7], SandboxActivityRefusal> {
    let declared = [
        u64::from_be_bytes(
            call[50..58]
                .try_into()
...
/// # Errors
///
/// Returns a refusal when hashing the canonical submission fails.
pub fn canonical_submission_digest(
    submission: &SandboxSubmission,
...
mod tests {
    ...
    fn malformed_and_replayed_sequences_are_not_canonical_submissions() {
        let protocol =
            SandboxProtocolContext::new(2, 3).unwrap_or_else(|error| panic!("protocol: {error:?}"));
        ...
        for index in 0..7 {
            call[50 + index * 8..58 + index * 8].copy_from_slice(&1u64.to_be_bytes());
        }
        call.push(b'x');
        ...
        payload[232..236].copy_from_slice(
            &u32::try_from(call.len())
                .unwrap_or_else(|error| panic!("call length: {error:?}"))
                .to_be_bytes(),
        );
        payload.extend_from_slice(&call);
        assert_eq!(
            SandboxSubmission::decode(&payload, protocol),
            Err(SandboxActivityRefusal::NonCanonical)
        );
        payload[36..44].copy_from_slice(&1u64.to_be_bytes());
        assert!(SandboxSubmission::decode(&payload, protocol).is_ok());
        assert_eq!(
            SandboxSubmission::decode(
                &payload,
                SandboxProtocolContext {
                    protocol_version: 1,
                    active_programs_abi: 3
                }
            ),
        ...
        assert_eq!(
            SandboxSubmission::decode(&payload, protocol),
            Err(SandboxActivityRefusal::CallMismatch)
        );
    }
}
