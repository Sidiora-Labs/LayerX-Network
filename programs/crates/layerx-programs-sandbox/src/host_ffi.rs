use layerx_programs_runtime::{
    hash_bytes, reserve_host_sandbox_escrow_charge, settle_reserved_host_sandbox_escrow_charge,
    ActivityBudgetBinding, HashAlgorithm, MeteredUsage, ProgramId, ReservedSandboxEscrowCharge,
};
...
unsafe extern "C" {
    ...
    fn layerx_programs_sandbox_destroy_archive(
        token: u64,
        kind: u16,
        bytes: *const u8,
        length: u32,
    ) -> i32;
    fn layerx_programs_sandbox_destroy_refund(
        token: u64,
        from: *const u8,
        to: *const u8,
        asset: *const u8,
    ...
    fn layerx_programs_sandbox_destroy_charge(
        token: u64,
        from: *const u8,
        to: *const u8,
        asset: *const u8,
...
fn destroy_state(token: u64, kind: u16) -> Result<Vec<u8>, i32> {
    ...
    (0..u32::try_from(length).map_err(|_| NON_CANONICAL)?)
        .map(|offset| {
...
fn read_destroy_state(
    token: u64,
    lease_id: [u8; 32],
    expected_root: [u8; 32],
    expected_sequence: u64,
    boundary: u64,
) -> Result<DurableUsageState, i32> {
    let lease = Lease::decode_state(&destroy_state(token, 0)?).map_err(|_| NON_CANONICAL)?;
    let escrow =
        Escrow::decode_state(&lease, &destroy_state(token, 1)?).map_err(|_| NON_CANONICAL)?;
    let ledger = UsageLedger::decode_state(&destroy_state(token, 2)?, &lease, &escrow)
        .map_err(|_| NON_CANONICAL)?;
    if lease.id().bytes() != lease_id
        || lease.state_digest().map_err(|_| NON_CANONICAL)? != expected_root
        || expected_sequence == 0
        || boundary < lease.expiry()
        || matches!(lease.state(), crate::LeaseState::Destroyed)
    {
        return Err(NON_CANONICAL);
    }
    Ok(DurableUsageState {
        lease,
        escrow,
        ledger,
    })
...
fn publish_destroy(
    ...
    state: &DurableUsageState,
    ledger_bytes: &[u8],
    usage_lease_bytes: &[u8],
    receipt_digest: [u8; 32],
    transfer_root: [u8; 32],
    amount: u128,
) -> Result<(), i32> {
    let lease_bytes = state
        .lease
        .canonical_state_bytes()
        .map_err(|_| NON_CANONICAL)?;
    let escrow_bytes = state.escrow.canonical_state();
    for (kind, bytes) in [
        (0u16, lease_bytes.as_slice()),
        (1, escrow_bytes.as_slice()),
        (2, ledger_bytes),
        (3, usage_lease_bytes),
    ] {
        if unsafe {
            layerx_programs_sandbox_destroy_archive(
                token,
                kind,
                bytes.as_ptr(),
                u32::try_from(bytes.len()).map_err(|_| NON_CANONICAL)?,
            )
        } != OK
        {
            return Err(NON_CANONICAL);
    ...
    terminal.extend_from_slice(&receipt_digest);
    terminal.extend_from_slice(&transfer_root);
    terminal.extend_from_slice(&amount.to_be_bytes());
    terminal.extend_from_slice(&state.lease.state_digest().map_err(|_| NON_CANONICAL)?);
    terminal.extend_from_slice(
        &hash_bytes(HashAlgorithm::Sha256, &escrow_bytes).map_err(|_| NON_CANONICAL)?,
    );
    DESTROY_TERMINAL.with(|slot| {
        *slot.borrow_mut() = Some(DestroyTerminal { bytes: terminal });
    });
    Ok(())
}

#[no_mangle]
pub extern "C" fn layerx_programs_sandbox_destroy_host(
    token: u64,
    lease_id: *const u8,
    ...
    std::panic::catch_unwind(|| {
        if lease_id.is_null() || expected_root.is_null() || activity_id.is_null() {
            return Err(NON_CANONICAL);
        }
        let mut id = [0u8; 32];
        let mut root = [0u8; 32];
        let mut activity = [0u8; 32];
        unsafe {
            id.copy_from_slice(core::slice::from_raw_parts(lease_id, 32));
            root.copy_from_slice(core::slice::from_raw_parts(expected_root, 32));
            activity.copy_from_slice(core::slice::from_raw_parts(activity_id, 32));
        }
        destroy_host(token, id, root, activity, expected_sequence, boundary)
    })
    .ok()
    .and_then(Result::ok)
    .map_or(NON_CANONICAL, |()| OK)
}

#[no_mangle]
pub extern "C" fn layerx_programs_sandbox_destroy_terminal(
    _token: u64,
    output: *mut u8,
    capacity: u32,
) -> i32 {
    ...
    DESTROY_TERMINAL.with(|slot| {
        ...
        if value.bytes.len() > capacity as usize {
            return NON_CANONICAL;
        }
        unsafe {
            core::ptr::copy_nonoverlapping(value.bytes.as_ptr(), output, value.bytes.len());
        }
        OK
    })
}

const fn low_word(value: u128) -> u64 {
...
fn admitted_charge(
    ...
    expiry: u64,
    occupancy_price: u64,
    maximum_execution: u128,
) -> Option<u128> {
    if maximum_execution == 0 || prior_batch >= expiry {
        return None;
    }
    maximum_execution.checked_add(occupancy_charge(
        namespace_bytes,
        expiry.checked_sub(prior_batch)?,
        occupancy_price,
    )?)
}

#[allow(clippy::too_many_arguments)]
fn postexecution_reserve_fits(
    remaining: u128,
    namespace_bytes: u64,
    ...
    expiry: u64,
    occupancy_price: u64,
    exact_execution: u128,
) -> bool {
    if prior_batch > observed_batch || observed_batch >= expiry {
        return false;
    }
    let Some(current) = occupancy_charge(
        namespace_bytes,
        observed_batch - prior_batch,
        occupancy_price,
    ) else {
        return false;
    };
    let Some(future) = occupancy_charge(
        final_namespace_bytes,
        expiry - observed_batch,
        occupancy_price,
    ) else {
        return false;
    };
    exact_execution
        .checked_add(current)
        .and_then(|value| value.checked_add(future))
...
unsafe extern "C" {
    ...
    fn layerx_programs_call_sandbox_usage_result_begin(
        ...
        occupancy_hi: u64,
        occupancy_lo: u64,
        occupancy_fee_hi: u64,
        occupancy_fee_lo: u64,
        transfer0: u64,
        transfer1: u64,
        transfer2: u64,
        transfer3: u64,
...
fn read_state(token: u64, kind: u16) -> Result<Vec<u8>, i32> {
    ...
    (0..length)
        .map(|offset| {
            let value = unsafe {
                layerx_programs_call_sandbox_state_byte(
                    token,
                    kind,
                    u32::try_from(offset).map_err(|_| NON_CANONICAL)?,
                )
            };
...
fn context_field(token: u64, section: u16) -> Result<[u8; 32], i32> {
    ...
    for (offset, byte) in bytes.iter_mut().enumerate() {
        *byte = u8::try_from(unsafe {
            layerx_programs_call_sandbox_context_byte(
                token,
                section,
                u32::try_from(offset).map_err(|_| NON_CANONICAL)?,
            )
...
fn stage(token: u64, kind: u16, bytes: &[u8]) -> Result<(), i32> {
    ...
    for (offset, byte) in bytes.iter().copied().enumerate() {
        if unsafe {
            layerx_programs_call_sandbox_state_stage_byte(
                token,
                kind,
                u32::try_from(offset).map_err(|_| NON_CANONICAL)?,
                byte,
...
pub extern "C" fn layerx_programs_sandbox_admit_host(
    ...
    admit_host(token, observed_batch, maximum_fee_hi, maximum_fee_lo)
...
fn admit_host(
    ...
    let escrow = Escrow::decode_state(&lease, &read_state(token, 1)?).map_err(|_| NON_CANONICAL)?;
    ...
    let prior_batch = ledger
        .latest()
        .map_or(lease.opened_at(), crate::UsageReceipt::observed_batch);
    if observed_batch < prior_batch || observed_batch >= lease.expiry() {
        return Err(NON_CANONICAL);
    }
    let maximum_execution = (u128::from(maximum_fee_hi) << 64) | u128::from(maximum_fee_lo);
    let remaining = escrow
        .funded()
        .checked_sub(escrow.spent())
        .and_then(|value| value.checked_sub(escrow.refunded()))
        .ok_or(NON_CANONICAL)?;
    let maximum_charge = admitted_charge(
        lease.usage().namespace_bytes,
        prior_batch,
        lease.expiry(),
        schedule.occupancy_byte_batch_price(),
        maximum_execution,
    ...
    .ok_or(NON_CANONICAL)?;
    if maximum_charge > remaining {
        return Err(NON_CANONICAL);
    }
    let activity_id: [u8; 32] = lifecycle_field(token, 8)?
        .try_into()
        .map_err(|_| NON_CANONICAL)?;
    let principal = lease
        .namespace()
    ...
    HOST_SETTLEMENT_RESERVATION.with(|slot| {
        ...
        *slot = Some(HostSettlementReservation {
            token,
            state: DurableUsageState {
                lease,
                escrow,
...
#[no_mangle]
#[allow(clippy::too_many_arguments)]
pub extern "C" fn layerx_programs_sandbox_final_reserve_host(
    token: u64,
    observed_batch: u64,
    cpu: u64,
    memory: u64,
...
fn validate_settlement_context(
    token: u64,
    reservation: &HostSettlementReservation,
    host: ProgramId,
) -> Result<(), i32> {
    let lease = &reservation.state.lease;
    let ledger = &reservation.state.ledger;
    if lease.host_program() != host
        || context_field(token, 0)? != lease.id().bytes()
        || context_field(token, 1)? != lease.escrow_account()
        || context_field(token, 2)? != lease.escrow_asset()
        || context_field(token, 3)? != lease.fee_destination()
...
fn settle_call(
    ...
    let binding =
        ActivityBudgetBinding::new(words_to_bytes(binding_words)).map_err(|_| NON_CANONICAL)?;
    let mut reservation = HOST_SETTLEMENT_RESERVATION
        .with(|slot| slot.borrow_mut().take())
        .ok_or(NON_CANONICAL)?;
    if reservation.token != token {
        return Err(NON_CANONICAL);
    }
    validate_settlement_context(token, &reservation, host)?;
    let lease = &reservation.state.lease;
    ...
    let occupancy_fee_units = occupancy_byte_batches
        .checked_mul(u128::from(schedule.occupancy_byte_batch_price()))
        .ok_or(NON_CANONICAL)?;
    ...
    let usage = MeteredUsage {
        cpu_fuel: cpu,
        memory_bytes: memory,
        storage_read_bytes: storage_read,
        storage_write_bytes: storage_write,
        output_values,
        output_bytes,
    ...
    let cumulative = cumulative_usage(lease, usage, outcome, final_namespace_bytes)?;
    let exact_fee = execution_fee
        .checked_add(occupancy_fee_units)
        .ok_or(NON_CANONICAL)?;
    escrow
        .permits_execution(lease, exact_fee)
        .map_err(|_| NON_CANONICAL)?;
    let settlement =
        settle_reserved_host_sandbox_escrow_charge(token, &mut reservation.transfer, exact_fee)
            .map_err(|_| NON_CANONICAL)?;
    ...
    let receipt = record_host_settlement_reserved(
        ...
        cumulative,
        observed_batch,
        settlement.transfer_set_root(),
        crate::usage::SettlementBindings {
            lease_terms_digest: *lease_terms_digest,
            expected_lease_digest: *expected_lease_digest,
        },
        crate::usage::SettlementBuffers {
            lease_state,
            receipt_bytes,
        },
    )
...
fn execution_fee(
    schedule: layerx_programs_runtime::FeeSchedule,
    units: [u64; 6],
) -> Result<u128, i32> {
    [
        (units[0], schedule.cpu_price()),
        (units[1], schedule.memory_byte_price()),
        (units[2], schedule.storage_read_byte_price()),
        (units[3], schedule.storage_write_byte_price()),
        (units[4], schedule.output_value_price()),
        (units[5], schedule.output_byte_price()),
    ]
    .into_iter()
    .try_fold(0u128, |total, (units, price)| {
        total
...
fn cumulative_usage(
    ...
    outcome: u8,
    final_namespace_bytes: u64,
) -> Result<LeaseUsage, i32> {
    Ok(LeaseUsage {
        cpu_fuel: lease
            .usage()
            .cpu_fuel
            .checked_add(usage.cpu_fuel)
            .ok_or(NON_CANONICAL)?,
        memory_bytes: lease.usage().memory_bytes.max(usage.memory_bytes),
        storage_read_bytes: lease
            .usage()
            .storage_read_bytes
            .checked_add(usage.storage_read_bytes)
            .ok_or(NON_CANONICAL)?,
        storage_write_bytes: lease
            .usage()
            .storage_write_bytes
            .checked_add(usage.storage_write_bytes)
            .ok_or(NON_CANONICAL)?,
        output_values: lease
            .usage()
            .output_values
            .checked_add(u64::from(usage.output_values))
            .ok_or(NON_CANONICAL)?,
        output_bytes: lease
            .usage()
            .output_bytes
...
fn publish_settlement(
    token: u64,
    reservation: &mut HostSettlementReservation,
    receipt: &crate::UsageReceipt,
    usage: MeteredUsage,
    transfer_root: [u8; 32],
) -> Result<(), i32> {
    let HostSettlementReservation {
    ...
    state
        .ledger
        .write_canonical_state(ledger_state)
        .map_err(|_| NON_CANONICAL)?;
    ...
    let root = words(transfer_root);
    if unsafe {
        layerx_programs_call_sandbox_usage_result_begin(
            token,
            (usage.occupancy_byte_batches >> 64) as u64,
            low_word(usage.occupancy_byte_batches),
            (usage.occupancy_fee_units >> 64) as u64,
            low_word(usage.occupancy_fee_units),
            root[0],
            root[1],
            root[2],
            root[3],
            u32::try_from(receipt_bytes.len()).map_err(|_| NON_CANONICAL)?,
        )
    } != OK
    {
        return Err(NON_CANONICAL);
    ...
    for (offset, byte) in receipt_bytes.iter().copied().enumerate() {
        if unsafe {
            layerx_programs_call_sandbox_usage_result_receipt_byte(
                token,
                u32::try_from(offset).map_err(|_| NON_CANONICAL)?,
                byte,
            )
    ...
    let fee_hi = u64::try_from(usage.fee_units >> 64).map_err(|_| NON_CANONICAL)?;
    let fee_lo = low_word(usage.fee_units);
    for (index, value) in [
        usage.cpu_fuel,
        usage.memory_bytes,
    ...
    {
        if unsafe {
            layerx_programs_call_sandbox_usage_result_field(
                token,
                u16::try_from(index).map_err(|_| NON_CANONICAL)?,
                value,
            )
        } != OK
        {
...
const fn words_to_bytes(words: [u64; 4]) -> [u8; 32] {
    ...
    while index < 4 {
        ...
        let mut offset = 0;
        while offset < 8 {
            bytes[index * 8 + offset] = word[offset];
            offset += 1;
        }
        index += 1;
    }
...
fn validate_lifecycle(token: u64, operation: u8) -> Result<(), i32> {
    if operation == 2 || operation == 0x82 {
        let mut lease =
            Lease::decode_state(&lifecycle_field(token, 0)?).map_err(|_| NON_CANONICAL)?;
        let tenant: [u8; 32] = lifecycle_field(token, 3)?
            .try_into()
            .map_err(|_| NON_CANONICAL)?;
        let host: [u8; 32] = lifecycle_field(token, 4)?
            .try_into()
            .map_err(|_| NON_CANONICAL)?;
        let amount: [u8; 16] = lifecycle_field(token, 5)?
            .try_into()
            .map_err(|_| NON_CANONICAL)?;
        let expiry: [u8; 8] = lifecycle_field(token, 6)?
            .try_into()
            .map_err(|_| NON_CANONICAL)?;
        if lease.state() != crate::LeaseState::Requested
            || lease.tenant().bytes() != tenant
            || lease.host_program().bytes() != host
            || lease.escrow_amount() != u128::from_be_bytes(amount)
            || lease.expiry() != u64::from_be_bytes(expiry)
        {
            return Err(NON_CANONICAL);
        }
        if operation == 0x82 {
            let funding_root: [u8; 32] = lifecycle_field(token, 7)?
                .try_into()
                .map_err(|_| NON_CANONICAL)?;
            let activity_id: [u8; 32] = lifecycle_field(token, 8)?
                .try_into()
                .map_err(|_| NON_CANONICAL)?;
            let batch = u64::from_be_bytes(
                lifecycle_field(token, 10)?
                    .try_into()
                    .map_err(|_| NON_CANONICAL)?,
            );
            lease
                .apply_host_activity(crate::LeaseActivity::Fund, activity_id, batch)
                .map_err(|_| NON_CANONICAL)?;
            ...
            stage(
                token,
                0,
                &lease.canonical_state_bytes().map_err(|_| NON_CANONICAL)?,
            )?;
            stage(token, 1, &escrow.canonical_state())?;
            stage(
                token,
                2,
                &ledger.canonical_state().map_err(|_| NON_CANONICAL)?,
            )?;
        }
    } else if operation == 3 {
        let current =
            Lease::decode_state(&lifecycle_field(token, 13)?).map_err(|_| NON_CANONICAL)?;
        let expected_digest: [u8; 32] = lifecycle_field(token, 11)?
            .try_into()
            .map_err(|_| NON_CANONICAL)?;
        let expected_sequence = u64::from_be_bytes(
            lifecycle_field(token, 12)?
                .try_into()
                .map_err(|_| NON_CANONICAL)?,
        );
        let activity_id: [u8; 32] = lifecycle_field(token, 8)?
        ...
        if current.state_digest().map_err(|_| NON_CANONICAL)? != expected_digest
            || expected_sequence
                != u64::try_from(current.history().len())
                    .map_err(|_| NON_CANONICAL)?
                    .checked_add(1)
                    .ok_or(NON_CANONICAL)?
        ...
        expected
            .apply_host_activity(crate::LeaseActivity::Activate, activity_id, batch)
            .map_err(|_| NON_CANONICAL)?;
        stage(
            token,
            0,
