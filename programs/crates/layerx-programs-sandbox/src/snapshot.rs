...
use layerx_programs_runtime::{
    hash_bytes, Abi, AuthorizationContext, ExecutionFault, HashAlgorithm, MeterRefusal,
    PrincipalId, ProgramInstance, RuntimeContinuation, RuntimeGlobal, Storage, StorageError,
    UnavailableReceiptOracle, ValidatedModule, WasmValue, ABI_V1_VERSION, ABI_V2_VERSION,
};

use crate::{
    EphemeralNamespace, Lease, LeaseActivity, LeaseId, LeaseRefusal, LeaseState, LeaseTransition,
    TransitionEvidence,
};
...
impl SandboxState {
    fn new(
        ...
        let state = Self {
            source_lease: lease.id(),
            source_namespace: lease.namespace(),
            linear_memory,
            globals,
            continuation,
            namespace_cells,
    ...
    fn from_runtime(lease: &Lease, runtime: RuntimeContinuation) -> Result<Self, SnapshotRefusal> {
        Self::new(
            lease,
            runtime.linear_memory,
            runtime.globals,
            ContinuationPoint {
    ...
    #[must_use]
    pub const fn source_namespace(&self) -> EphemeralNamespace {
        self.source_namespace
    }
    #[must_use]
    pub fn linear_memory(&self) -> &[u8] {
        &self.linear_memory
    }
    #[must_use]
    ...
    #[must_use]
    pub const fn continuation(&self) -> &ContinuationPoint {
        &self.continuation
    }
    #[must_use]
    pub fn namespace_cells(&self) -> &[NamespaceCell] {
    ...
    fn validate_against(&self, lease: &Lease) -> Result<(), SnapshotRefusal> {
        ...
        let namespace_bytes = self.namespace_cells.iter().try_fold(0u64, |total, cell| {
            let key =
                u64::try_from(cell.key.len()).map_err(|_| SnapshotRefusal::StateBoundExceeded)?;
            let value =
                u64::try_from(cell.value.len()).map_err(|_| SnapshotRefusal::StateBoundExceeded)?;
            total
                .checked_add(key)
    ...
    pub fn canonical_bytes(&self) -> Result<Vec<u8>, SnapshotRefusal> {
        ...
        for value in &self.continuation.arguments {
...
impl<'a> SnapshotCursor<'a> {
    ...
    fn take(&mut self, length: usize) -> Result<&'a [u8], SnapshotRefusal> {
        ...
        let value = self
            .bytes
            .get(self.offset..end)
            .ok_or(SnapshotRefusal::NonCanonicalContinuation)?;
        self.offset = end;
        Ok(value)
    }
...
impl Snapshot {
    ...
    pub fn commit(
        ...
        if transition.activity != LeaseActivity::Snapshot
            || transition.from != LeaseState::Active
            || transition.to != LeaseState::Active
            || transition.usage_observation_digest != digest
        {
        ...
        candidate
            .snapshot_transition(transition, evidence)
            .map_err(SnapshotRefusal::Lease)?;
        candidate
            .bind_snapshot(digest, lease.tenant(), byte_length, chunk_count)
            .map_err(SnapshotRefusal::Lease)?;
        let mut candidate_storage = instance
            .storage_snapshot()
            .ok_or(SnapshotRefusal::MissingRuntimeStorage)?;
        candidate_storage
            .replace_protocol_prefix(
                lease.namespace().snapshot_storage_namespace(),
        ...
        let occupied = candidate_storage
            ...
            .ok_or(SnapshotRefusal::StateBoundExceeded)?;
        if occupied > lease.limits().namespace_bytes {
            return Err(SnapshotRefusal::TargetBoundExceeded);
        }
        ...
        *lease = candidate;
        Ok(Self {
            ...
            image_code_hash: lease.image_code_hash(),
            byte_length,
            storage_bytes,
            state: captured,
        })
...
impl RestoredSandbox {
    #[must_use]
    pub const fn target_lease(&self) -> LeaseId {
        self.target_lease
    }
    #[must_use]
...
pub fn restore(
    ...
    candidate_meter
        .charge_storage_write(
            byte_length
                .checked_add(entries_metered_bytes(&entries)?)
                .ok_or(SnapshotRefusal::StateBoundExceeded)?,
        )
    ...
    .map_err(SnapshotRefusal::Abi)?;
    let mut instance = module
        .instantiate_sandbox(candidate_meter, abi)
        .map_err(SnapshotRefusal::Runtime)?;
    let outputs = instance
        .restore_continuation(&state.runtime_continuation())
        .map_err(SnapshotRefusal::Runtime)?;
    let committed_storage = instance
        .storage_snapshot()
        .ok_or(SnapshotRefusal::MissingRuntimeStorage)?;
    let committed_meter = instance.meter().clone();
    let restored = RestoredSandbox {
        target_lease: target.id(),
...
fn live_namespace_cells(
    ...
    storage
        ...
        .map_err(SnapshotRefusal::Storage)?
        .into_iter()
        .try_fold(Vec::new(), |mut cells, (key, value)| {
            let cell = NamespaceCell { key, value };
...
fn rebound_live_entries(cells: &[NamespaceCell]) -> Result<NamespaceEntries, SnapshotRefusal> {
    ...
    for cell in cells {
        ...
        entries.push((cell.key.clone(), cell.value.clone()));
    }
    Ok(entries)
}
...
fn verify_persisted_snapshot(
    ...
    let prefix = snapshot_prefix(digest);
    let entries = storage
        .protocol_prefix_entries(namespace.snapshot_storage_namespace(), &prefix)
        .map_err(SnapshotRefusal::Storage)?;
    let Some((_, manifest)) = entries.first() else {
        return Err(SnapshotRefusal::DigestMismatch);
    };
    let expected_length =
        u64::try_from(expected.len()).map_err(|_| SnapshotRefusal::StateBoundExceeded)?;
    let stored_chunks = usize::try_from(u32::from_be_bytes(
        manifest
            .get(40..44)
            .ok_or(SnapshotRefusal::DigestMismatch)?
            .try_into()
    ...
    let restored: Vec<u8> = entries
        .iter()
        .skip(1)
        .flat_map(|(_, value)| value.iter().copied())
        .collect();
    if restored != expected
        || hash_bytes(HashAlgorithm::Sha256, &restored).map_err(|_| SnapshotRefusal::HashRefusal)?
            != digest
    {
        return Err(SnapshotRefusal::DigestMismatch);
    }
