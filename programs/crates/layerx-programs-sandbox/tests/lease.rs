...
use layerx_programs_runtime::{
    AuthorizationContext, CapabilitySet, FeeSchedule, Meter, PrincipalId, ProgramId,
    ResourceBudget, Storage,
};
use layerx_programs_sandbox::{
    usage_observation_digest, Lease, LeaseActivity, LeaseBook, LeaseId, LeaseLimits, LeaseRefusal,
    LeaseState, LeaseTransition, LeaseUsage, TransitionEvidence, UsageOutcome,
...
fn verified(
    lease: &Lease,
    mut transition: LeaseTransition,
    batch: u64,
...
fn wrong_principal_refusal(
    ...
    let head = verifier
        ...
        .unwrap_or_else(|error| panic!("verified head: {error}"));
    transition.activity_id = head.activity_id();
    TransitionEvidence::verify_call(
        &head,
        lease,
        transition,
    ...
    .err()
    .unwrap_or_else(|| panic!("wrong principal was accepted"))
}
...
fn real_receipts_drive_every_lifecycle_edge_and_destroyed_never_revives() {
    ...
    let request_digest = lease
        .request_binding_digest()
        .unwrap_or_else(|error| panic!("request digest: {error}"));
    let unverified_request = LeaseTransition {
        lease: lease.id(),
        tenant: lease.tenant(),
        activity: LeaseActivity::Request,
        from: LeaseState::Requested,
        to: LeaseState::Requested,
        activity_id: [0; 32],
        usage_observation_digest: request_digest,
    };
    assert_eq!(
        wrong_principal_refusal(&lease, unverified_request, 10),
        LeaseRefusal::TenantMismatch
    );
    let (request, request_evidence) = transition(
        &lease,
    ...
    drive_lifecycle_edges(&mut book);
    let destroyed = book
        .get(LeaseId::new([1; 32]).unwrap_or_else(|error| panic!("id: {error}")))
        .unwrap_or_else(|| panic!("destroyed lease"))
        .clone();
    let (revive, evidence) = transition(
        &destroyed,
        LeaseActivity::Fund,
    ...
    assert_eq!(
        book.transition(destroyed.id(), revive, evidence),
...
fn real_bound_receipt_closes_intrinsically_and_refuses_mismatch_and_regression() {
    let mut book = activate_bound_receipt_lease();
    ...
    assert_eq!(
        ...
        Ok(UsageOutcome::Recorded(LeaseUsage {
            cpu_fuel: 5,
            ..LeaseUsage::default()
        }))
    );
    ...
    let current = book
        .get(id)
        .unwrap_or_else(|| panic!("active lease"))
        .clone();
    let usage = LeaseUsage {
        cpu_fuel: 11,
        ..LeaseUsage::default()
    };
    let digest = usage_observation_digest(id, usage, 5, 14)
        .unwrap_or_else(|error| panic!("usage digest: {error}"));
    let (close, evidence) = transition(
        &current,
    ...
    assert_eq!(
        book.transition(id, close, evidence),
        Err(LeaseRefusal::IntrinsicActivityRequired)
    );
    let (close, evidence) = transition(
        &current,
        LeaseActivity::CloseBoundExceeded,
    ...
    let before = book
        .get(id)
        .unwrap_or_else(|| panic!("before refusal"))
        .state_digest()
        .unwrap_or_else(|error| panic!("before digest: {error}"));
    let mut altered = close;
    altered.to = LeaseState::Expired;
    assert_eq!(
        book.record_usage(id, usage, 5, 14, Some(&(altered, evidence))),
        Err(LeaseRefusal::ActivityReceiptMismatch)
    );
    assert_eq!(
        book.get(id)
            .unwrap_or_else(|| panic!("after refusal"))
            .state_digest(),
        Ok(before)
    );
    assert!(matches!(
        book.record_usage(id, usage, 5, 14, Some(&(close, evidence))),
...
fn real_request_evidence_enforces_principal_concurrency_and_expiry() {
    let mut book = fill_principal_and_refuse_corrupt_state();
    let first = LeaseId::new([1; 32]).unwrap_or_else(|error| panic!("first id: {error}"));
    let current = book
        .get(first)
        .unwrap_or_else(|| panic!("first lease"))
        .clone();
    ...
    book.transition(first, expire, evidence)
    ...
    let current = book
        .get(first)
        .unwrap_or_else(|| panic!("expired lease"))
        .clone();
    let (destroy, evidence) = transition(
        &current,
    ...
    let lease = candidate(34, 9, 22, 30);
    let digest = lease
        .request_binding_digest()
        .unwrap_or_else(|error| panic!("digest: {error}"));
    let (request, evidence) = transition(
    ...
    expire_and_destroy_remaining_leases(&mut book);
    refill_principal_and_refuse_overflow(&mut book);
}
...
fn lease_prefix_is_host_accessible_isolated_and_addressable_as_one_unit() {
    ...
    let storage_namespace = namespace
        .storage_namespace()
...
fn altered_real_activity_and_header_evidence_are_refused() {
    ...
    let digest = lease
        .request_binding_digest()
        .unwrap_or_else(|error| panic!("digest: {error}"));
    let mut transition = LeaseTransition {
        lease: lease.id(),
        tenant: lease.tenant(),
        activity: LeaseActivity::Request,
        from: LeaseState::Requested,
    ...
    let last = activity
        .len()
        .checked_sub(1)
        .unwrap_or_else(|| panic!("activity bytes"));
    activity[last] ^= 1;
    assert_eq!(
        TransitionEvidence::verify_call(
            &head,
    ...
    let last = header
        .len()
        .checked_sub(1)
        .unwrap_or_else(|| panic!("header bytes"));
    header[last] ^= 1;
    assert_eq!(
        TransitionEvidence::verify_call(
            &head,
...
fn drive_lifecycle_edges(book: &mut LeaseBook) {
    for (activity, from, to, batch) in [
        ...
        let current = book
            .get(LeaseId::new([1; 32]).unwrap_or_else(|error| panic!("id: {error}")))
            .unwrap_or_else(|| panic!("stored lease"))
            .clone();
        ...
        if activity == LeaseActivity::Destroy {
            assert_eq!(
                ...
                Err(LeaseRefusal::ReplayedEvidence)
            );
        } else {
            assert_eq!(
                book.transition(current.id(), declared, evidence),
...
fn activate_bound_receipt_lease() -> LeaseBook {
    ...
    for (activity, from, to, batch) in [
        ...
        let current = book
            .get(LeaseId::new([2; 32]).unwrap_or_else(|error| panic!("id: {error}")))
            .unwrap_or_else(|| panic!("lease"))
            .clone();
        if current.state() == LeaseState::Funded {
            let (expire, expire_evidence) = transition(
                &current,
                LeaseActivity::Expire,
                LeaseState::Funded,
                LeaseState::Expired,
            ...
            assert!(matches!(
                book.transition(current.id(), expire, expire_evidence),
        ...
        if activity == LeaseActivity::Activate {
            ...
            altered.to = LeaseState::Expired;
            assert_eq!(
                book.transition(current.id(), altered, evidence),
                Err(LeaseRefusal::ActivityReceiptMismatch)
            );
        }
        book.transition(current.id(), declared, evidence)
...
fn fill_principal_and_refuse_corrupt_state() -> LeaseBook {
    let mut book = LeaseBook::new();
    for index in 1..=u8::try_from(MAX_CONCURRENT_LEASES_PER_PRINCIPAL)
        .unwrap_or_else(|error| panic!("lease limit: {error}"))
    {
    ...
    let requested = book
        .get(LeaseId::new([2; 32]).unwrap_or_else(|error| panic!("requested id: {error}")))
...
fn expire_and_destroy_remaining_leases(book: &mut LeaseBook) {
    for index in 2..=33 {
        let id = LeaseId::new([index; 32]).unwrap_or_else(|error| panic!("id: {error}"));
...
fn refill_principal_and_refuse_overflow(book: &mut LeaseBook) {
    for index in 34..34
        + u8::try_from(MAX_CONCURRENT_LEASES_PER_PRINCIPAL)
            .unwrap_or_else(|error| panic!("lease limit: {error}"))
    {
        let lease = candidate(index, 9, 32, 40);
        let digest = lease
            .request_binding_digest()
            .unwrap_or_else(|error| panic!("digest: {error}"));
        let (request, evidence) = transition(
        ...
        book.insert_requested(lease, request, evidence)
            .unwrap_or_else(|error| panic!("refill: {error}"));
    }
    let lease = candidate(66, 9, 32, 40);
    let digest = lease
        .request_binding_digest()
        .unwrap_or_else(|error| panic!("digest: {error}"));
    let (request, evidence) = transition(
