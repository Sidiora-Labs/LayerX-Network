...
import (
	...
	"github.com/sidiora-labs/paxeer-network/engine/deps/tasks"
	...
	authrest "github.com/sidiora-labs/paxeer-network/sdk/x/auth/client/rest"
	authkeeper "github.com/sidiora-labs/paxeer-network/sdk/x/auth/keeper"
	authsims "github.com/sidiora-labs/paxeer-network/sdk/x/auth/simulation"
	...
	authzmodule "github.com/sidiora-labs/paxeer-network/sdk/x/authz/module"
	...
	distr "github.com/sidiora-labs/paxeer-network/sdk/x/distribution"
	distrclient "github.com/sidiora-labs/paxeer-network/sdk/x/distribution/client"
	...
	feegrantmodule "github.com/sidiora-labs/paxeer-network/sdk/x/feegrant/module"
	...
	paramsclient "github.com/sidiora-labs/paxeer-network/sdk/x/params/client"
	...
	upgradeclient "github.com/sidiora-labs/paxeer-network/sdk/x/upgrade/client"
	...
	gigaexecutor "github.com/sidiora-labs/paxeer-network/engine/executor"
	gigaconfig "github.com/sidiora-labs/paxeer-network/engine/executor/config"
	gigalib "github.com/sidiora-labs/paxeer-network/engine/executor/lib"
	...
	"github.com/sidiora-labs/paxeer-network/interchain/modules/apps/transfer"
	...
	ibc "github.com/sidiora-labs/paxeer-network/interchain/modules/core"
	ibcclient "github.com/sidiora-labs/paxeer-network/interchain/modules/core/02-client"
	ibcclientclient "github.com/sidiora-labs/paxeer-network/interchain/modules/core/02-client/client"
	ibcclienttypes "github.com/sidiora-labs/paxeer-network/interchain/modules/core/02-client/types"
	ibcporttypes "github.com/sidiora-labs/paxeer-network/interchain/modules/core/05-port/types"
	...
	appante "github.com/sidiora-labs/paxeer-network/node/ante"
	...
	epochmodule "github.com/sidiora-labs/paxeer-network/modules/epoch"
	...
	oraclemodule "github.com/sidiora-labs/paxeer-network/modules/oracle"
	...
	wasmclient "github.com/sidiora-labs/paxeer-network/wasm/x/wasm/client"
	...
	_ "github.com/sidiora-labs/paxeer-network/docs/swagger"
	...
	gigabankkeeper "github.com/sidiora-labs/paxeer-network/engine/deps/xbank/keeper"
	gigaevmkeeper "github.com/sidiora-labs/paxeer-network/engine/deps/xevm/keeper"
...
var (
	...
	// names mounted on the PaxDB / memiavl backend. It is the single source
	// of truth consumed by sdk.NewKVStoreKeys in app.New, and is cross-
	// checked against keys.MemIAVLStoreKeys (storage/common/keys) by tests
	// in this package. Adding or removing an entry here MUST be matched in
	// storage/common/keys/store_keys.go.
	...
	// Boolean to only emit paxd version and git commit metric once per chain initialization
	EmittedPaxdVersionMetric = false
...
func newGigaBlockCache(ctx sdk.Context, keeper *gigaevmkeeper.Keeper) (*gigaBlockCache, error) {
	...
	sstore := keeper.GetParams(ctx).PaxSstoreSetGasEip2200
...
func New(
	...
	bAppOptions, stateStore := SetupPaxDB(homePath, appOpts, baseAppOptions)
	...
	app.LayerXAnchorKeeper = layerxanchorkeeper.NewKeeper(app.keys[layerxanchortypes.StoreKey], app.AccountKeeper, app.BankKeeper)

	// The last arguments can contain custom message handlers, and custom query handlers,
	// if we want to allow any custom callbacks
	supportedFeatures := "iterator,staking,stargate,pax"
	wasmOpts = append(
	...
	// invocation in consensus/internal/state/execution.go next to
	...
	govRouter.AddRoute(govtypes.RouterKey, govtypes.ProposalHandler).
		AddRoute(paramproposal.RouterKey, evm.NewParamChangeProposalHandler(&app.EvmKeeper, app.ParamsKeeper)).
	...
	app.txPrioritizer = NewPaxTxPrioritizer(&app.EvmKeeper, &app.UpgradeKeeper, &app.ParamsKeeper).GetTxPriorityHint
...
func (app *App) GetStateStore() paxdb.StateStore { return app.stateStore }
...
func (app *App) executeEVMTxWithGigaExecutor(ctx sdk.Context, msg *evmtypes.MsgEVMTransaction, cache *gigaBlockCache) (*abci.ExecTxResult, error) {
	...
	sender, paxAddr, _, recoverErr := evmante.RecoverSenderFromEthTx(ctx, ethTx, chainID)
	if recoverErr != nil {
		return &abci.ExecTxResult{
			Code: 1,
	...
	_, isAssociated := app.GigaEvmKeeper.GetEVMAddress(ctx, paxAddr)

	// Run validation checks (fee/nonce/balance - stateless checks done earlier)
	validation := app.validateGigaEVMTx(ctx, ethTx, sender, paxAddr, isAssociated)
	...
	if execErr != nil {
		...
		// happens inside go-ethereum's Execute() rather than the Pax
...
func (app *App) RPCContextProvider(i int64) sdk.Context {
	if i == evmrpc.LatestCtxHeight {
		...
		// nil from any RPC handler. rpc/block.go's gasLimit and
		// rpc/info.go's gas-used-ratio rely on it.
...
func (app *App) validateGigaEVMTx(
	...
	senderBalance := app.GigaEvmKeeper.GetBalance(ctx, paxAddr)
