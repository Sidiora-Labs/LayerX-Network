defmodule Explorer.Chain.Import.Runner.PaxeerX.Anchors do
  @moduledoc """
  Bulk imports `t:Explorer.Chain.PaxeerX.Anchor.t/0`.
  ...
  @type imported :: [Anchor.t()]

  @impl Import.Runner
  def ecto_schema_module, do: Anchor

  @impl Import.Runner
  def option_key, do: :paxeer_x_anchors
  ...
  def run(multi, changes_list, %{timestamps: timestamps} = options) do
    ...
    Multi.run(multi, :insert_paxeer_x_anchors, fn repo, _ ->
  ...
  def insert(repo, changes_list, %{timeout: timeout, timestamps: timestamps} = _options) when is_list(changes_list) do
    # Enforce PaxeerX.Anchor ShareLocks order (see docs: sharelock.md)
