defmodule Explorer.Chain.Import.Runner.PaxeerX.MarketEvents do
  @moduledoc """
  Bulk imports `t:Explorer.Chain.PaxeerX.MarketEvent.t/0`.
  ...
  @type imported :: [MarketEvent.t()]

  @impl Import.Runner
  def ecto_schema_module, do: MarketEvent

  @impl Import.Runner
  def option_key, do: :paxeer_x_market_events
  ...
  def run(multi, changes_list, %{timestamps: timestamps} = options) do
    ...
    Multi.run(multi, :insert_paxeer_x_market_events, fn repo, _ ->
  ...
  def insert(repo, changes_list, %{timeout: timeout, timestamps: timestamps} = _options) when is_list(changes_list) do
    # Enforce PaxeerX.MarketEvent ShareLocks order (see docs: sharelock.md)
