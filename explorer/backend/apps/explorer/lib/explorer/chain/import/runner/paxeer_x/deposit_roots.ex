defmodule Explorer.Chain.Import.Runner.PaxeerX.DepositRoots do
  @moduledoc """
  Bulk imports `t:Explorer.Chain.PaxeerX.DepositRoot.t/0`.
  ...
  alias Explorer.Chain.PaxeerX.DepositRoot
  ...
  @type imported :: [DepositRoot.t()]

  @impl Import.Runner
  def ecto_schema_module, do: DepositRoot

  @impl Import.Runner
  def option_key, do: :paxeer_x_deposit_roots
  ...
  def run(multi, changes_list, %{timestamps: timestamps} = options) do
    ...
    Multi.run(multi, :insert_paxeer_x_deposit_roots, fn repo, _ ->
  ...
  def insert(repo, changes_list, %{timeout: timeout, timestamps: timestamps} = _options) when is_list(changes_list) do
    # Enforce PaxeerX.DepositRoot ShareLocks order (see docs: sharelock.md)
