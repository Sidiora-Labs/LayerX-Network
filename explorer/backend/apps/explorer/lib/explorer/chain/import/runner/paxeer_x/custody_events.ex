defmodule Explorer.Chain.Import.Runner.PaxeerX.CustodyEvents do
  @moduledoc """
  Bulk imports `t:Explorer.Chain.PaxeerX.CustodyEvent.t/0`.
  ...
  @type imported :: [CustodyEvent.t()]

  @impl Import.Runner
  def ecto_schema_module, do: CustodyEvent

  @impl Import.Runner
  def option_key, do: :paxeer_x_custody_events
  ...
  def run(multi, changes_list, %{timestamps: timestamps} = options) do
    ...
    Multi.run(multi, :insert_paxeer_x_custody_events, fn repo, _ ->
  ...
  def insert(repo, changes_list, %{timeout: timeout, timestamps: timestamps} = _options) when is_list(changes_list) do
    # Enforce PaxeerX.CustodyEvent ShareLocks order (see docs: sharelock.md)
